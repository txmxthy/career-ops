...
4. **Results** are automatically merged into `data/applications.md`, processed offers are reconciled out of the `data/pipeline.md` inbox, and integrity is verified with `src/scripts/verify-pipeline.mjs` at the end of the run.
...
4. After all workers finish, batch-runner calls `merge-tracker.mjs` to merge TSVs into `data/applications.md`, `src/scripts/reconcile-pipeline.mjs` to move processed offers out of the `data/pipeline.md` inbox, and `src/scripts/verify-pipeline.mjs` to check integrity.
