/**
 * src/lib/liveness-browser.mjs — Playwright-driven liveness check for a single URL.
 *
 * Shared by src/scripts/check-liveness.mjs (CLI tool) and scan.mjs (--verify flag).
...
// allowed out. Exported so other Playwright callers (src/scripts/archive-posting.mjs) wire
...
export async function checkUrlLiveness(page, url, { extraSettleMs = 0 } = {}) {
  ...
  try {
    ...
    // test doubles in tests/run-all.mjs do — and such a caller must keep getting the
