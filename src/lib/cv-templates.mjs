#!/usr/bin/env node
// src/lib/cv-templates.mjs — discover, resolve, and validate CV / cover-letter templates.
...
if (isMain) {
  ...
  try {
    if (cmd === 'list') {
      ...
      process.stderr.write('Usage: node src/lib/cv-templates.mjs <list|resolve> <cv|cover> [name] [--format=html|tex] [--fallback]\n');
      process.exit(2);
    }
  } catch (err) {
