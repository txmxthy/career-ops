...
/**
 * src/lib/patch-latex-content.mjs — Apply prose patches to a user-owned LaTeX CV in place.
 *
 * Usage:
 *   node src/lib/patch-latex-content.mjs <source.tex> <patches.json> <output.tex>
 *
 * patches.json:
 *   { "patches": [ { "id": "bullet-0", "text": "Tailored bullet text" } ] }
 *
 * Optional manifest fields in patches.json (from src/lib/extract-latex-content.mjs):
 *   { "slots": [...], "patches": [...] }
 */
...
async function main() {
  ...
  if (!sourcePath || !patchesPath || !outputPath) {
    console.error('Usage: node src/lib/patch-latex-content.mjs <source.tex> <patches.json> <output.tex>');
    process.exit(1);
  }
  ...
  if (slots.length === 0) {
    console.error('patches.json must include a slots array from src/lib/extract-latex-content.mjs');
    process.exit(1);
  }
