...
/**
 * src/scripts/jd-skill-gap.mjs — Zero-LLM JD skill-gap checker.
 *
 ...
 *
 * Usage:
 *   node src/scripts/jd-skill-gap.mjs jds/acme.md
 *   node src/scripts/jd-skill-gap.mjs jds/acme.md --summary
 *   node src/scripts/jd-skill-gap.mjs --self-test
 */
...
// as src/scripts/upskill.mjs's SKILL_PATTERN, where (?!\w) is equivalent to \b for
 ...
 * failure the shared src/lib/skill-extract.mjs module was introduced to end.
 *
 * @param {string} jdText
 ...
 * src/lib/skill-extract.mjs knows both names. STOPWORDS can also empty a bullet. All the
...
function classifySkillGaps(jdSkills, cvText) {
  ...
  // of being reported as a false gap. This is the shared tokenizer src/scripts/upskill.mjs
...
`;
  ...
  // same bug src/scripts/upskill.mjs's SKILL_PATTERN fixed with a (?!\w) lookahead. The
...
`;
  ...
  // extracts nothing even when src/lib/skill-extract.mjs knows every name in it. Same
...
if (selfTestMode) {
  ...
  if (!jdPathArg || !existsSync(jdPathArg)) {
    console.error('Usage: node src/scripts/jd-skill-gap.mjs <jd-file> [--summary]');
    console.error('       node src/scripts/jd-skill-gap.mjs --self-test');
    process.exit(1);
  }
