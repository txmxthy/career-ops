...
// @ts-check
/**
 * src/scripts/plugins.mjs — explicit CLI host for the non-provider plugin hooks.
 *
 *   node src/scripts/plugins.mjs list                       # discovered plugins + status
 *   node src/scripts/plugins.mjs run <id> [hook] [args…]    # run one hook of one plugin
 *   node src/scripts/plugins.mjs run gmail                  # ingest (the plugin's only hook)
 *   node src/scripts/plugins.mjs run notion search "staff platform engineer"
 *   node src/scripts/plugins.mjs run notion export [--dry-run]
 *
...
import { parseRow, isSeparatorRow } from '../core/table.js';
...
async function cmdList() {
  ...
  console.log('\nEnable in config/plugins.yml, add keys to .env, then `node src/scripts/plugins.mjs run <id>`.');
}
...
async function cmdRun(args) {
  ...
  if (!id) { console.error('Usage: node src/scripts/plugins.mjs run <id> [hook] [args…] [--dry-run]'); process.exit(1); }

  const cfg = await loadPluginConfig(ROOT);
  const manifest = discoverPlugins(pluginRoots(ROOT), resolveSuccessorIds(ROOT)).find(m => m.id === id);
  if (!manifest) { console.error(`Unknown plugin "${id}". Run \`node src/scripts/plugins.mjs list\`.`); process.exit(1); }
  ...
  if (!hook) {
    ...
    else { console.error(`Plugin "${id}" exposes multiple hooks (${runnable.join(', ')}). Specify one: node src/scripts/plugins.mjs run ${id} <hook>`); process.exit(1); }
  }
  ...
  if (hook === 'ingest' || hook === 'search') {
    ...
    if (hook === 'search' && !payload) { console.error(`search needs a query: node src/scripts/plugins.mjs run ${id} search "<query>"`); process.exit(1); }
...
function capabilityCard(manifest, source) {
  return [
    ...
    `  Ships a skill: ${manifest.skill ? 'yes — instructs your AI tool when you run `src/scripts/plugins.mjs skill`' : 'no'}`,
  ].join('\n');
}
...
async function cmdAvailable() {
  ...
  if (reg.plugins.length === 0) {
    ...
    console.log('\nInstall:  node src/scripts/plugins.mjs add <name>');
  }
}
...
function cmdSkill(args) {
  ...
  if (!id) {
    ...
    console.log(withSkill.length ? 'Plugins that ship a skill:\n' + withSkill.map(m => `  ${m.id}`).join('\n') + '\n\nRead one: node src/scripts/plugins.mjs skill <id>' : 'No installed plugin ships a skill.');
    return;
  }
...
function cmdEnable(args) {
  ...
  if (!id) { console.error('Usage: node src/scripts/plugins.mjs enable <id> [--confirm]'); process.exit(1); }
  const m = findManifest(id);
  if (!m) { console.error(`Unknown plugin "${id}". Run \`node src/scripts/plugins.mjs list\`.`); process.exit(1); }
...
function cmdRemove(args) {
  const id = args[0];
  if (!id) { console.error('Usage: node src/scripts/plugins.mjs remove <id>'); process.exit(1); }
...
function cmdNew(args) {
  const name = args[0];
  if (!name) { console.error('Usage: node src/scripts/plugins.mjs new <name>'); process.exit(1); }
  let dest;
  ...
  console.log(`    A) develop locally:  node src/scripts/plugins.mjs enable ${name}`);
...
async function cmdAdd(args) {
  ...
  const confirm = hasFlag(args, '--confirm');
  if (!target) { console.error('Usage: node src/scripts/plugins.mjs add <name|owner/repo> [--sha <commit>] [--confirm]'); process.exit(1); }
  ...
  else console.log(`\n  To enable it (grants the above), run:  node src/scripts/plugins.mjs enable ${installed.id} --confirm`);
}
...
async function main() {
  ...
  switch (cmd) {
    ...
    default:
      console.error('Usage: node src/scripts/plugins.mjs [list | available | run <id> [hook] | skill <id> | new <name> | add <name|owner/repo> [--sha <c>] [--confirm] | enable <id> [--confirm] | trust <id> | remove <id>]');
      process.exit(1);
  }
}
