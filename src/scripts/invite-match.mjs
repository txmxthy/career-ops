#!/usr/bin/env node
/**
 * src/scripts/invite-match.mjs — Interview-Invite / Rejection → Tracker Matcher for career-ops
 *
 ...
 * script that already references `src/scripts/invite-match.mjs` (e.g. #1495's own
 * history) — left as a maintainer call, not blocking.
 *
 * Run: node src/scripts/invite-match.mjs < invite.txt          (JSON to stdout)
 *      node src/scripts/invite-match.mjs --file invite.txt
 *      echo "..." | node src/scripts/invite-match.mjs --summary
 *      node src/scripts/invite-match.mjs --apply [--id N]      (rejection-classified matches only; advances status to Rejected)
 *      node src/scripts/invite-match.mjs --self-test
 *
...
const USAGE = `Usage:
  node src/scripts/invite-match.mjs < invite.txt          # JSON to stdout
  node src/scripts/invite-match.mjs --file invite.txt     # read invite text from a file
  node src/scripts/invite-match.mjs --summary             # human-readable summary instead of JSON
  node src/scripts/invite-match.mjs --apply [--id N]      # rejection-classified matches only; advances status to Rejected
  node src/scripts/invite-match.mjs --self-test           # run the built-in self-test suite
  node src/scripts/invite-match.mjs --help                # print this usage block and exit
  node src/scripts/invite-match.mjs -h                    # same as --help`;

// Only when invoked as a CLI, not when another script (scan.mjs,
// src/scripts/detect-reposts.mjs) imports a helper like normalizeCompanyName — otherwise
...
// src/scripts/detect-reposts.mjs --window). Same condition as the "Run" guard at the
// bottom of this file.
//
 ...
 * stricter than src/scripts/dedup-tracker.mjs's normalizeCompany (which only lowercases
