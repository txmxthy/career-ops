#!/usr/bin/env node
/**
 * src/scripts/upskill.mjs — Aggregate skill-gap analyzer for career-ops (#1520, phase 1)
 *
 ...
 *
 * Run: node src/scripts/upskill.mjs            (JSON to stdout)
 *      node src/scripts/upskill.mjs --summary  (human-readable table)
 *      node src/scripts/upskill.mjs --min-reports 3
...
// guard in src/scripts/analyze-patterns.mjs so both sites behave the same.
...
// Mirrors src/scripts/analyze-patterns.mjs (duplicated by design, see header comment).
...
`);
  ...
  // the raw string to normalizeJd.
  {
    ...
    if (/normalizeJd\(\s*targetText/.test(selfSrc)) {
      failures.push('url-text: src/scripts/upskill.mjs still passes the raw fetched string to normalizeJd (regression, #1894)');
    }
...
// --- CLI ---
// Everything below runs ONLY when src/scripts/upskill.mjs is the process entry point.
//
...
//
// Under tests/ it also broke the harness, because tests/run-all.mjs imports discovered
...
//
// Same shape as the other CLIs in this repo (src/scripts/add-entry.mjs, src/scripts/detect-reposts.mjs,
// src/scripts/contacts.mjs, src/scripts/check-table-freshness.mjs, ...): compare import.meta.url against
...
// nothing — the same edge tests/contacts.test.mjs documents on macOS. Every caller
// (tests/run-all.mjs, the modes, package scripts) uses the real path, and matching the
...
const USAGE = `Usage:
  node src/scripts/upskill.mjs                        # aggregate skill-gap map (JSON)
  node src/scripts/upskill.mjs --summary              # human-readable table
  node src/scripts/upskill.mjs --min-reports <n>      # minimum scored reports required (default 5)
  node src/scripts/upskill.mjs --url-text <url|path>  # targeted gap analysis vs one JD (URL or local file)
  node src/scripts/upskill.mjs <url>                  # same as --url-text <url>
  node src/scripts/upskill.mjs --self-test            # run the pure-function self-tests
  node src/scripts/upskill.mjs --help                 # show this message`;
