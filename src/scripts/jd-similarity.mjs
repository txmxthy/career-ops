#!/usr/bin/env node
/**
 * src/scripts/jd-similarity.mjs — deterministic CV reuse recommendation for similar JDs.
 *
 * This is deliberately a recommendation layer. It never evaluates a JD and
 * never deletes or overwrites an existing CV.
 *
 * Usage:
 *   node src/scripts/jd-similarity.mjs new-jd.txt previous-jd-or-cv.txt
 */
...
const USAGE = `Usage:
  node src/scripts/jd-similarity.mjs new-jd.txt previous-jd-or-cv.txt
  node src/scripts/jd-similarity.mjs --help                    # print this usage block and exit`;
