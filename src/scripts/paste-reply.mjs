...
/**
 * src/scripts/paste-reply.mjs — manual, no-Gmail input path into the src/scripts/reply-watch.mjs
 * classification pipeline (#1802).
 *
 * src/scripts/reply-watch.mjs already classifies employer replies (Interview / Responded /
 ...
 * into the exact candidate object shape src/scripts/reply-watch.mjs expects, and append it
 * to data/reply-candidates.json. It does NOT classify the reply — classification
 * stays src/scripts/reply-watch.mjs's job. src/scripts/reply-matcher.mjs's classifyReply() derives its
 ...
 * src/scripts/reply-watch.mjs itself and never touches data/applications.md.
 *
 * Usage:
 *   node src/scripts/paste-reply.mjs                  # interactive: prompts for subject, from, body
 *   node src/scripts/paste-reply.mjs --file email.txt # read subject/from/body from a file
 *   node src/scripts/paste-reply.mjs --help
 *
 ...
 *                                 this script, matching src/scripts/reply-watch.mjs's default)
 */
 ...
 * src/scripts/reply-watch.mjs expects. Exported for direct unit testing.
 */
export function normalizeCandidate({ subject, from, body }) {
  return {
    ...
    subject: subject || '',
    body_snippet: body || '',
    // Classification is src/scripts/reply-watch.mjs's job, not this script's — leaving
...
function printHelp() {
  console.log(`src/scripts/paste-reply.mjs — manual/no-Gmail input into the src/scripts/reply-watch.mjs pipeline (#1802)

Usage:
  node src/scripts/paste-reply.mjs                  interactive: prompts for subject, from, body
  node src/scripts/paste-reply.mjs --file <path>    read subject/from/body from a file
  node src/scripts/paste-reply.mjs --help
...
this script never runs src/scripts/reply-watch.mjs or touches data/applications.md itself.`);
}
...
async function main() {
  ...
  console.log('Next: run `node src/scripts/reply-watch.mjs` to classify and review this reply.');
}

// Only run when executed directly (`node src/scripts/paste-reply.mjs`), not when imported
