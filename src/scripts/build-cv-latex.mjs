...
async function main() {
  ...
  if (args.length === 0 || args.includes('--help')) {
    console.error('Usage:');
    console.error('  node src/scripts/build-cv-latex.mjs <input.json> <output.tex>');
    console.error('  node src/scripts/build-cv-latex.mjs --test');
    process.exit(1);
  }
  ...
  if (!inputPath || !outputPath) {
    console.error('Usage: node src/scripts/build-cv-latex.mjs <input.json> <output.tex>');
    process.exit(1);
  }
