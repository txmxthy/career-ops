#!/usr/bin/env node
/**
 * src/scripts/ollama-eval.mjs — Ollama-powered Job Offer Evaluator for career-ops
 *
 ...
 *
 * Usage:
 *   node src/scripts/ollama-eval.mjs "Paste full JD text here"
 *   node src/scripts/ollama-eval.mjs --file ./jds/my-job.txt
 *   node src/scripts/ollama-eval.mjs --model qwen2.5:72b --file ./jds/my-job.txt
 *
 * Requires:
...
╚══════════════════════════════════════════════════════════════════╝
  ...
  USAGE
    node src/scripts/ollama-eval.mjs "<JD text>"
    node src/scripts/ollama-eval.mjs --file ./jds/my-job.txt
    node src/scripts/ollama-eval.mjs --model qwen2.5:72b "<JD text>"
  ...
  EXAMPLES
    node src/scripts/ollama-eval.mjs "We are looking for a Senior AI Engineer..."
    node src/scripts/ollama-eval.mjs --file ./jds/openai-swe.txt
    OLLAMA_MODEL=mistral-nemo node src/scripts/ollama-eval.mjs --file ./jds/job.txt
`);
  process.exit(0);
}
...
❌  Remote Ollama endpoint detected: ${baseUrl}
   ...
   Ollama on a home server), set:
     OLLAMA_ALLOW_REMOTE=1 node src/scripts/ollama-eval.mjs ...
`);
    process.exit(1);
  }
}
