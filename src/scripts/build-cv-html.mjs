...
// Deterministic HTML CV renderer (#557 — the HTML twin of src/scripts/build-cv-latex.mjs).
//
...
// src/lib/cv-templates.mjs to honor config-selectable templates, #1691) — it owns every
...
// files stays in the agent (same contract as src/scripts/build-cv-latex.mjs / modes/latex.md).
...
async function main() {
  ...
  if (args.length === 0 || args.includes('--help')) {
    console.error('Usage:');
    console.error('  node src/scripts/build-cv-html.mjs <input.json> <output.html> [template.html]');
    console.error('  node src/scripts/build-cv-html.mjs --preview <input.json> [template.html]');
    console.error('  node src/scripts/build-cv-html.mjs --test');
    console.error('');
    console.error('  [template.html] defaults to templates/cv-template.html. Pass the path');
    console.error('  printed by `node src/lib/cv-templates.mjs resolve cv` to use a selected template.');
    console.error('');
  ...
  if (!inputPath || !outputPath) {
    console.error('Usage: node src/scripts/build-cv-html.mjs <input.json> <output.html> [template.html]');
    process.exit(1);
  }
