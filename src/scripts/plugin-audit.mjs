#!/usr/bin/env node
// @ts-check
// src/scripts/plugin-audit.mjs — static safety scan for COMMUNITY/registry plugins (run by
// `src/scripts/plugins.mjs add` and by the registry-validate CI). Lives at repo ROOT (not
...
const USAGE = `Usage:
  node src/scripts/plugin-audit.mjs <plugin-dir>
  node src/scripts/plugin-audit.mjs --help`;
...
// CLI: node src/scripts/plugin-audit.mjs <dir>
