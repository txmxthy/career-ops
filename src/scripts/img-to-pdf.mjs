...
/**
 * src/scripts/img-to-pdf.mjs — Screenshot/image -> single-page PDF via Playwright
 *
 ...
 *
 * Usage:
 *   node src/scripts/img-to-pdf.mjs <image-path> <output-path> [--force]
...
const SPEC = {
  command: 'img-to-pdf',
  usage: 'node src/scripts/img-to-pdf.mjs <image-path> <output-path> [--force]',
  flags: {
...
function usage() {
  console.log('Usage: node src/scripts/img-to-pdf.mjs <image-path> <output-path> [--force]');
  console.log('');
