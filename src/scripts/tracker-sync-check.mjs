#!/usr/bin/env node
/**
 * src/scripts/tracker-sync-check.mjs — applications.md <-> active-interviews.md status
 ...
 *   2. Fallback fuzzy match — Company + Role text, using src/scripts/invite-match.mjs's
 *      normalizeCompanyName/companySimilarity (imported directly, not
 *      copied) plus src/lib/role-matcher.mjs's roleFuzzyMatch (already shared with
 *      detect-reposts.mjs/src/scripts/dedup-tracker.mjs).
 ...
 *
 * Run: node src/scripts/tracker-sync-check.mjs              (JSON to stdout)
 *      node src/scripts/tracker-sync-check.mjs --summary    (human-readable table)
 *      node src/scripts/tracker-sync-check.mjs --apps-file path/to/applications.md
 *      node src/scripts/tracker-sync-check.mjs --interviews-file path/to/active-interviews.md
 *      node src/scripts/tracker-sync-check.mjs --self-test
 *
...
// Mirrors the ALIASES map in src/scripts/analyze-patterns.mjs / src/scripts/verify-pipeline.mjs —
...
// src/scripts/invite-match.mjs (issue #1495 / PR #1497), which is now on main — see the
...
// src/lib/role-matcher.mjs's roleFuzzyMatch agreeing on the role text.
...
//
// Mirrors the table-scan in src/scripts/process-quality.mjs's parseActiveInterviews
...
//
// Those two rules are still a hand-mirror of src/scripts/process-quality.mjs and must stay
...
// Case-insensitive column lookup — same rationale as src/scripts/process-quality.mjs's
 ...
 * AND src/lib/role-matcher.mjs's roleFuzzyMatch agrees, and exactly one such
 ...
 * the function src/scripts/verify-pipeline.mjs imports rather than shelling out, so the
...
function runSelfTest() {
  ...
  // --- normalizeCompanyName / companySimilarity (imported from src/scripts/invite-match.mjs) ---
  ...
  // src/scripts/invite-match.mjs implementation uses \p{L}\p{M}\p{N} (any script) instead,
  ...
  // slightly differently but src/lib/role-matcher.mjs's roleFuzzyMatch still agrees
