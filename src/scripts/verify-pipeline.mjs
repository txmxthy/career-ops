#!/usr/bin/env node
/**
 * src/scripts/verify-pipeline.mjs — Health check for career-ops pipeline integrity
 *
 * Checks:
 ...
 *
 * Run: node career-ops/src/scripts/verify-pipeline.mjs
 */
...
// Delegates to src/scripts/tracker-sync-check.mjs's exported checkTrackerSync() rather than
...
// a fix (src/scripts/tracker-sync-check.mjs is intentionally reporting-only for now — see
// its module header).
let syncResult;
try {
...
if (syncResult) {
  ...
  for (const m of tier1Mismatches) {
    warn(`Sync drift (auto-resolvable): ${m.company} — ${m.role}: applications.md="${m.applicationsStatus}" vs active-interviews.md="${m.activeInterviewsStatus}" -> suggest "${m.suggestedStatus}" in ${m.staleIn} (run node src/scripts/tracker-sync-check.mjs for details)`);
  }
...
// src/scripts/stats.mjs (computeFollowupStats) and followup-cadence.mjs both read this table
...
if (!existsSync(FOLLOWUPS_FILE)) {
  ...
  if (fups.pipeLines === 0) {
    ...
    error(`follow-ups.md: none of its ${fups.dataRows} row(s) parse, so src/scripts/stats.mjs and followup-cadence.mjs will both report zero follow-ups — expected column order ${FOLLOWUPS_COLUMNS} (see modes/followup.md)`);
  } else if (fups.unparsedLines.length > 0) {
    warn(`follow-ups.md: ${fups.unparsedLines.length} of ${fups.dataRows} rows will be skipped by src/scripts/stats.mjs and followup-cadence.mjs (line${fups.unparsedLines.length === 1 ? '' : 's'} ${fups.unparsedLines.join(', ')}) — expected ${FOLLOWUPS_COLUMNS}`);
  } else {
