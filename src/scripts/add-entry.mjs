#!/usr/bin/env node
/**
 * src/scripts/add-entry.mjs — Deterministic dedup + insertion for `/career-ops add`.
 *
 ...
 *
 * Usage:
 *   node src/scripts/add-entry.mjs <payload.json> [--dry-run]
 *   node src/scripts/add-entry.mjs --stdin [--dry-run]        (read payload JSON from stdin)
 *
...
const USAGE = `Usage:
  node src/scripts/add-entry.mjs <payload.json> [--dry-run]
  node src/scripts/add-entry.mjs --stdin [--dry-run]
  node src/scripts/add-entry.mjs --help                    # print this usage block and exit (-h is an alias)`;
