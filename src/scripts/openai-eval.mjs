#!/usr/bin/env node
/**
 * src/scripts/openai-eval.mjs — OpenAI-compatible Job Offer Evaluator for career-ops
 *
 ...
 * via --file. Mirrors src/scripts/ollama-eval.mjs / gemini-eval.mjs.
 *
 * Usage:
 *   node src/scripts/openai-eval.mjs "Paste full JD text here"
 *   node src/scripts/openai-eval.mjs --file ./jds/my-job.txt
 *   node src/scripts/openai-eval.mjs --url https://openrouter.ai/api/v1 --model meta-llama/llama-3.3-70b-instruct --file ./jds/job.txt
 *
 ...
 * --url at http://localhost:... (or use src/scripts/ollama-eval.mjs).
 */
...
╚══════════════════════════════════════════════════════════════════╝
  ...
  USAGE
    node src/scripts/openai-eval.mjs "<JD text>"
    node src/scripts/openai-eval.mjs --file ./jds/my-job.txt
    node src/scripts/openai-eval.mjs --url <base> --model <id> --file ./jds/job.txt
  ...
  EXAMPLES
    OPENAI_API_KEY=sk-... node src/scripts/openai-eval.mjs --file ./jds/job.txt
    node src/scripts/openai-eval.mjs --url http://localhost:1234/v1 --model local "<JD text>"
`);
  process.exit(0);
}
...
❌  No API key for ${endpointHost}.
   ...
   Set one and re-run:
     OPENAI_API_KEY=your_key node src/scripts/openai-eval.mjs ...
