#!/usr/bin/env node
/**
 * src/scripts/contacts.mjs — Job-search phonebook → vCard 3.0 exporter
 *
 ...
 *
 * Run: node src/scripts/contacts.mjs                     (JSON: contacts + quality + total)
 *      node src/scripts/contacts.mjs --summary           (human-readable table)
 *      node src/scripts/contacts.mjs --vcf [path]        (write vCard, default output/contacts.vcf)
...
const USAGE = `Usage:
  ...
  node src/scripts/contacts.mjs --vcf [path]        # write vCard, default output/contacts.vcf
  node src/scripts/contacts.mjs --vcf --caller-id   # FN as "Jane Doe (Acme recruiter)"
  node src/scripts/contacts.mjs --self-test         # run the in-memory test suite
...
function printSummary(contacts, quality) {
  ...
  if (!contacts.length) {
    ...
    console.log('  Export to your phone with: node src/scripts/contacts.mjs --vcf');
  } else {
...
function writeVcf(contacts, quality) {
  ...
  if (issues) {
    console.error(`⚠ ${issues} data-quality issue${issues === 1 ? '' : 's'} in data/contacts.tsv (details: node src/scripts/contacts.mjs --summary):`);
  ...
  // src/scripts/reconcile-pipeline.mjs / followup-seed.mjs realpath-containment pattern.
