#!/usr/bin/env node
/**
 * src/scripts/rejection-latency.mjs — Post-Interview Response-Latency Signal for career-ops
 *
 ...
 *      or src/lib/role-matcher.mjs fuzzy match; company-only fallback when the
 ...
 * opt-in guarantee).
 *
 * Run: node src/scripts/rejection-latency.mjs             (JSON to stdout)
 *      node src/scripts/rejection-latency.mjs --summary   (human-readable table)
 ...
 *      node src/scripts/rejection-latency.mjs --file path/to/active-interviews.md
 *      node src/scripts/rejection-latency.mjs --tracker path/to/applications.md
...
const USAGE = `Usage:
  node src/scripts/rejection-latency.mjs                        # JSON report
  node src/scripts/rejection-latency.mjs --summary              # human-readable table
  node src/scripts/rejection-latency.mjs --file <path>          # a different active-interviews.md
  node src/scripts/rejection-latency.mjs --tracker <path>       # a different applications.md
  node src/scripts/rejection-latency.mjs --courtesy-days <N>    # override the courtesy threshold (default ${DEFAULT_COURTESY_DAYS})
  node src/scripts/rejection-latency.mjs --today <YYYY-MM-DD>   # evaluate as of a fixed date
  node src/scripts/rejection-latency.mjs --self-test            # run the built-in fixtures
  node src/scripts/rejection-latency.mjs --help                 # show this message
...
// --- Date helpers (same conventions as src/scripts/detect-reposts.mjs) ---
...
export function computeRejectionLatency(interviewRows, trackerByCompany, opts = {}) {
  ...
  // differed even though src/lib/role-matcher.mjs's fuzzy match resolves both to
...
function runSelfTest() {
  ...
  // Role-aware join: fuzzy title variants still match (src/lib/role-matcher.mjs).
