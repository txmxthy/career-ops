#!/usr/bin/env node
// @ts-check
// src/scripts/plugin-install.mjs — clone/scaffold/validate community plugins. Lives at repo
...
export function installFromRepo(root, { url, sha }) {
  ...
  if (existsSync(dest)) throw new Error(`plugins.local/${id} already exists — \`node src/scripts/plugins.mjs remove ${id}\` first`);
