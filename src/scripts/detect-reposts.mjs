#!/usr/bin/env node
/**
 * src/scripts/detect-reposts.mjs — Repost Detector for career-ops
 *
 ...
 *
 * Run: node src/scripts/detect-reposts.mjs             (JSON to stdout)
 *      node src/scripts/detect-reposts.mjs --summary   (human-readable table)
 *      node src/scripts/detect-reposts.mjs --window 60 (override 90-day window)
 *      node src/scripts/detect-reposts.mjs --min-span 7 (override 1-day minimum span)
 *      node src/scripts/detect-reposts.mjs --self-test
 *      node src/scripts/detect-reposts.mjs --help
 *
...
const USAGE = `Usage:
  node src/scripts/detect-reposts.mjs                       # full JSON repost clusters to stdout
  node src/scripts/detect-reposts.mjs --summary             # human-readable table
  node src/scripts/detect-reposts.mjs --window 60           # override the default 90-day window
  node src/scripts/detect-reposts.mjs --min-span 7          # override the default 1-day minimum span
  node src/scripts/detect-reposts.mjs --self-test           # run the in-memory test suite
  node src/scripts/detect-reposts.mjs --help                # print this usage block and exit`;
...
// has to see. src/scripts/company-history.mjs renders a card for a flagged company even
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  //
  // Inside the main-module guard, not at import time: src/scripts/company-history.mjs
