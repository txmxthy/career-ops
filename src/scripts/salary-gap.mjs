#!/usr/bin/env node
/**
 * src/scripts/salary-gap.mjs — Desired vs Advertised vs Actual compensation analyzer
 *
 ...
 *
 * Run: node src/scripts/salary-gap.mjs             (JSON)
 *      node src/scripts/salary-gap.mjs --summary   (human-readable)
 *      node src/scripts/salary-gap.mjs --stated-for <tracker#>   (prior stated-comp observations, JSON)
...
import { hasFlag, flagValue } from '../core/flags.js';
...
// Like src/scripts/analyze-patterns.mjs:110 but WITHOUT `json`: analyze-patterns feeds the fence
...
function main() {
  ...
  if (hasFlag(args, '--stated-for')) {
    if (!statedForNum) {
      console.error('Usage: node src/scripts/salary-gap.mjs --stated-for <tracker#>');
      process.exit(1);
    }
