#!/usr/bin/env node
/**
 * src/scripts/browser-extract.mjs — headless Playwright reader for the scan / JD-extraction
 ...
 *
 * Usage:
 *   node src/scripts/browser-extract.mjs <url> [--mode jd|listing] [--max N] [--max-chars N] [--timeout MS]
 *
 ...
 * to the MCP path silently). Reuses src/lib/liveness-browser.mjs's SSRF host guard and
...
const USAGE_SYNOPSIS = 'src/scripts/browser-extract.mjs <url> [--mode jd|listing] [--max N] [--max-chars N] [--timeout MS]';
