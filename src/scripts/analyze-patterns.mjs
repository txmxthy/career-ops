#!/usr/bin/env node
/**
 * src/scripts/analyze-patterns.mjs — Rejection Pattern Detector for career-ops
 *
 ...
 *
 * Run: node src/scripts/analyze-patterns.mjs          (JSON to stdout)
 *      node src/scripts/analyze-patterns.mjs --summary (human-readable table)
 *      node src/scripts/analyze-patterns.mjs --min-threshold 3
 *      node src/scripts/analyze-patterns.mjs --min-vendor-n 8   (per-vendor sample floor)
...
// --- Status normalization (mirrors src/scripts/verify-pipeline.mjs) ---
const ALIASES = {
...
// src/lib/skill-extract.mjs and src/scripts/upskill.mjs already carry. Ordered longest-first
...
// src/scripts/upskill.mjs so both sites behave the same.
...
// Host-only match, deliberately looser than src/lib/liveness-api.mjs's resolveAtsApi()
