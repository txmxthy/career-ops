...
/**
 * src/scripts/generate-latex.mjs — Validate and compile a generated .tex CV file to PDF
 *
 * Usage:
 *   node src/scripts/generate-latex.mjs <input.tex> [output.pdf]
 *   node src/scripts/generate-latex.mjs <input.tex> [output.pdf] --compile-only
 *
...
async function main() {
  ...
  if (!inputPath) {
    console.error('Usage: node src/scripts/generate-latex.mjs <input.tex> [output.pdf] [--compile-only]');
    process.exit(1);
  }
