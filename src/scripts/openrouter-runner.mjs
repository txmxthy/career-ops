...
// `rawOverride` lets tests feed YAML text directly (see tests/run-all.mjs drift guard).
// ---------------------------------------------------------------------------
...
async function cmdScan() {
  ...
  if (added > 0) {
    console.log('\n→  node src/scripts/openrouter-runner.mjs pipeline\n   to evaluate pending listings.\n');
  }
}
...
// Entry point
// ---------------------------------------------------------------------------
// Only run the CLI when invoked directly (`node src/scripts/openrouter-runner.mjs ...`), so the
// module can be imported (e.g. by tests/run-all.mjs) without executing a command.
...
if (invokedDirectly) switch (command) {
  ...
  case 'apply':
    if (!args[0]) { console.error('Usage: node src/scripts/openrouter-runner.mjs apply <report_num|report_path>'); break; }
...
COMMANDS:
  node src/scripts/openrouter-runner.mjs scan              → Scan Greenhouse APIs for new matching listings
  node src/scripts/openrouter-runner.mjs evaluate <url>    → Evaluate a listing by URL
  node src/scripts/openrouter-runner.mjs evaluate          → Paste a job description interactively
  node src/scripts/openrouter-runner.mjs pipeline          → Batch-evaluate all pending entries in pipeline.md
  node src/scripts/openrouter-runner.mjs apply <report_no> → Generate application form answers from a report
  node src/scripts/openrouter-runner.mjs models            → List available free models from OpenRouter
...
MODEL SELECTION:
  ...
  - Pin a model:  CAREER_OPS_MODEL=deepseek/deepseek-r1:free node src/scripts/openrouter-runner.mjs eval <url>
`);
}
