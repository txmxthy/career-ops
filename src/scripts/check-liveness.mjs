...
/**
 * src/scripts/check-liveness.mjs — Playwright job link liveness checker
 *
 ...
 * (src/lib/liveness-api.mjs, no browser), then Playwright for everything else.
 *
 * Usage:
 *   node src/scripts/check-liveness.mjs <url1> [url2] ...
 *   node src/scripts/check-liveness.mjs --file urls.txt
 *
...
import {
  ...
  jitteredDelayMs,
  sleep,
} from '../lib/liveness-browser.mjs';
import { checkLivenessViaApi } from '../lib/liveness-api.mjs';
...
async function main() {
  ...
  if (positional.length === 0) {
    console.error('Usage: node src/scripts/check-liveness.mjs [--no-fallback] [--throttle[=ms]] <url1> [url2] ...');
    console.error('       node src/scripts/check-liveness.mjs [--no-fallback] [--throttle[=ms]] --file urls.txt');
    process.exit(1);
  }
