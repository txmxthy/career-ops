...
/**
 * src/scripts/story-provenance-check.mjs — Zero-LLM story-bank provenance checker.
 *
 * Companion to src/scripts/jd-skill-gap.mjs (read that file first — this one deliberately
 * follows its shape: regex-based extraction, no LLM call, classify-and-report
 * only, never auto-edits the user's data files). Where src/scripts/jd-skill-gap.mjs
 ...
 *
 * ── Why four buckets, not src/scripts/jd-skill-gap.mjs's three ────────────────────
 *
 * src/scripts/jd-skill-gap.mjs only ever asks "does cv.md have a trace of this or not" —
 * a binary trace/no-trace question is enough there because nothing is ever
 * auto-added to cv.md either way. This problem needs a state src/scripts/jd-skill-gap.mjs
 ...
 *
 * src/lib/match-star.mjs's parser already reads story-bank.md as `### [Theme] Title`
 ...
 *
 * Deliberately not exhaustive — matches src/scripts/jd-skill-gap.mjs's design stance
 ...
 * and low-noise — same tradeoff src/scripts/jd-skill-gap.mjs's SKILL_TOKEN_RE makes):
 *   - spelled-out numbers ("fifty students")
 *   - currency figures ("$40K budget") — src/scripts/verify-cv-facts.mjs already owns
 ...
 *   - non-ASCII digit scripts (src/scripts/verify-cv-facts.mjs's foldDigits() solves
 ...
 *
 * Usage:
 *   node src/scripts/story-provenance-check.mjs
 *   node src/scripts/story-provenance-check.mjs --summary
 *   node src/scripts/story-provenance-check.mjs --story-bank path/to/file.md --cv path/to/cv.md
 *   node src/scripts/story-provenance-check.mjs --self-test
 */
...
// ── Story-bank parsing ──────────────────────────────────────────────
// Same block shape src/lib/match-star.mjs's parseStories() already relies on
// (`### [Theme] Title` headers, `**Label:** value` lines) — reimplemented
// narrowly here rather than imported, so this checker doesn't take on a
// dependency on src/lib/match-star.mjs's STAR-specific fields it doesn't need.
 ...
 * as "scanned and clean" — same purpose as src/scripts/jd-skill-gap.mjs's
 * diagnoseExtraction().
...
// ── Exports (for tests/run-all.mjs and other consumers) ───────────────────
