#!/usr/bin/env node
/**
 * src/scripts/archive-posting.mjs — Save a live job posting as PDF before it disappears.
 *
 ...
 *
 * Usage:
 *   node src/scripts/archive-posting.mjs <url>
 *   node src/scripts/archive-posting.mjs <url> --company=Anthropic --role=senior-ai-engineer
 *   node src/scripts/archive-posting.mjs <url> --report=042    Key the capture to report #42
 *   node src/scripts/archive-posting.mjs --pipeline          Archive pending URLs in data/pipeline.md
 *   node src/scripts/archive-posting.mjs --dry-run <url>     Preview filename without saving
 *
 ...
 * resolving the next day. src/scripts/outcome.mjs looks captures up by report number.
 */
...
╚══════════════════════════════════════════════════════════════════╝
  ...
  USAGE
    node src/scripts/archive-posting.mjs <url>
    node src/scripts/archive-posting.mjs <url> --company=Anthropic --role=senior-ai-engineer
    node src/scripts/archive-posting.mjs --pipeline     Archive all pending URLs in data/pipeline.md
    node src/scripts/archive-posting.mjs --dry-run <url>
  ...
  WHY --report
    ...
    next day. src/scripts/outcome.mjs looks captures up by report number.

  PIPELINE REFERENCE (paste into pipeline.md or reports/)
    local:jds/{filename}

  EXAMPLES
    node src/scripts/archive-posting.mjs "https://jobs.ashbyhq.com/anthropic/abc123"
    node src/scripts/archive-posting.mjs "https://boards.greenhouse.io/openai/jobs/456" --company=OpenAI
    node src/scripts/archive-posting.mjs "https://jobs.lever.co/acme/xyz" --report=42
    node src/scripts/archive-posting.mjs --pipeline
...
function parseCliArgs(args) {
  ...
  // this file is imported for its exports (tests/run-all.mjs does), and a
