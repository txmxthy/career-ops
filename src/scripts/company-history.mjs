#!/usr/bin/env node
/**
 * src/scripts/company-history.mjs — Per-Company Evidence-Card Aggregator for career-ops
 *
 ...
 *     not reposting, and src/scripts/detect-reposts.mjs enforces that with a minimum span
 ...
 *   - status-log:    src/scripts/funnel-velocity.mjs, loaded ONLY via a dynamic
 ...
 *                     `typeof x === 'function'`.
 *
 * Run: node src/scripts/company-history.mjs                    (JSON to stdout)
 *      node src/scripts/company-history.mjs --summary           (human-readable cards)
 *      node src/scripts/company-history.mjs --company "Acme"    (single-card lookup)
 *      node src/scripts/company-history.mjs --silence-window 21 (override default window)
 *      node src/scripts/company-history.mjs --include-stale     (include >365d-old facts in labels)
...
const USAGE = `Usage:
  ...
  node src/scripts/company-history.mjs --summary              # human-readable cards
  node src/scripts/company-history.mjs --company "Acme"       # single-card lookup
  node src/scripts/company-history.mjs --silence-window 21    # override the default silence window (days)
  node src/scripts/company-history.mjs --include-stale        # include facts older than 365d in label computation
  node src/scripts/company-history.mjs --self-test            # run the in-memory test suite
  node src/scripts/company-history.mjs --summary --emit-signal  # human-readable cards, then RFC #1506 schema-v1 no-response-friction
  ...
  node src/scripts/company-history.mjs --help                 # print this usage block and exit`;
...
function parseArgs(argv) {
  ...
  if (emitSignal && (!summaryMode || company)) {
    console.error('Error: --emit-signal requires --summary and cannot be combined with --company (default and --company mode already print a single JSON document; appending JSON Lines after it would break JSON parsing of stdout). Use: node src/scripts/company-history.mjs --summary --emit-signal');
    console.error(USAGE);
    process.exit(1);
  }
...
export function loadRepostClusters(rootDir = CAREER_OPS, overridePath, portalsPath) {
  ...
  // Same override scan.mjs and src/scripts/detect-reposts.mjs honour, so a sandboxed run
...
// Dynamic-only dependency: src/scripts/funnel-velocity.mjs ships on main, but the
...
export async function loadStatusLogSource() {
  ...
  // whether the import resolved. The merged src/scripts/funnel-velocity.mjs exports neither
  ...
  for (const row of Array.isArray(rows) ? rows : []) {
    ...
    if (normalized === 'applied') {
      ...
      facts.push({
        ...
        // response date in the status ledger — the file src/scripts/funnel-velocity.mjs
...
// src/scripts/company-history.mjs already computes above (computeResponsiveness()) rather
...
// src/scripts/company-history.mjs already computes two independent axes
...
// src/scripts/discover-ats.mjs / src/scripts/check-table-freshness.mjs shape), not a bare array.
...
async function runSelfTest() {
  ...
  {
    // The merged src/scripts/funnel-velocity.mjs imports fine but exports neither optional
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  if (selfTestMode) {
    runSelfTest().catch(err => {
      console.error(`src/scripts/company-history.mjs: self-test error: ${err?.message || err}`);
      process.exit(1);
    });
  } else {
    const run = async () => {
      ...
      if (emitSignal) {
        ...
        for (const warning of warnings) {
          console.error(`src/scripts/company-history.mjs: ${warning}`);
        }
    ...
    run().catch(err => {
      console.error(`src/scripts/company-history.mjs: unexpected error: ${err?.message || err}`);
      process.exit(1);
    });
  }
}
