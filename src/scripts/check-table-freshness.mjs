#!/usr/bin/env node
/**
 * src/scripts/check-table-freshness.mjs — Staleness validator for jurisdiction data tables
 *
 ...
 * fails the run.
 *
 * Run: node src/scripts/check-table-freshness.mjs                    (JSON to stdout)
...
const USAGE = `Usage:
  node src/scripts/check-table-freshness.mjs                       # JSON freshness report
  node src/scripts/check-table-freshness.mjs --summary             # human-readable table
  node src/scripts/check-table-freshness.mjs --max-age-months <n>  # review-due threshold (default: 12)
  node src/scripts/check-table-freshness.mjs --today <YYYY-MM-DD>  # evaluate as of a fixed date
  node src/scripts/check-table-freshness.mjs --self-test           # run the inline self-test
  node src/scripts/check-table-freshness.mjs --help                # show this message
...
// --- Config (mirrors src/scripts/salary-gap.mjs's profile read: optional, never fatal) ---
