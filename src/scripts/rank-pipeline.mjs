#!/usr/bin/env node
/**
 * src/scripts/rank-pipeline.mjs — opt-in LLM relevance re-ranker for `data/pipeline.md` (#1144)
 *
 ...
 *
 * Usage:
 *   node src/scripts/rank-pipeline.mjs                     # rank up to --limit pending entries
 *   node src/scripts/rank-pipeline.mjs --limit 10
 *   node src/scripts/rank-pipeline.mjs --cli codex         # override CLI auto-detection
 *   node src/scripts/rank-pipeline.mjs --model <name>      # passed through when the CLI takes one
 *   node src/scripts/rank-pipeline.mjs --dry-run           # print what would be written
 *   node src/scripts/rank-pipeline.mjs --self-test         # in-memory suite; spawns no subprocess
 */
...
import { resolvePipelinePath } from '../core/store.js';
...
const USAGE = `
  src/scripts/rank-pipeline.mjs — opt-in LLM relevance re-ranker (annotates, never filters)

  node src/scripts/rank-pipeline.mjs [--limit N] [--cli <name>] [--model <name>] [--dry-run]
