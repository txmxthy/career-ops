...
/**
 * src/scripts/agent-inbox.mjs — a tiny bridge between *looking at* the pipeline and
 * *acting on* it.
 *
 ...
 *   node src/scripts/agent-inbox.mjs list [--all]                 # pending only, or every item
...
async function add() {
  ...
  if (!text) fail('add needs a request, e.g. node src/scripts/agent-inbox.mjs add "evaluate https://..."');
  ...
  // src/lib/pipeline-lock.mjs cuts the collision rate ~6x but is explicitly "not a
...
function fail(msg) {
  process.stderr.write(`src/scripts/agent-inbox.mjs: ${msg}\n`);
  process.exit(1);
}
...
if (cmd === 'add') await add();
  ...
  process.stdout.write(
    'Usage:\n' +
    '  node src/scripts/agent-inbox.mjs add "evaluate https://acme.com/jobs/42"\n' +
    '  node src/scripts/agent-inbox.mjs list [--all]\n' +
    '  node src/scripts/agent-inbox.mjs resolve <n> [--result "..."]\n',
  );
}
