#!/usr/bin/env node
/**
 * src/scripts/generate-cover-letter.mjs — Renders a cover letter payload to PDF.
 *
 * Usage:
 *   node src/scripts/generate-cover-letter.mjs --payload payload.json
...
export function buildHtml(payload, templatePath) {
  ...
  // a custom cover-letter template (KINDS.cover in src/lib/cv-templates.mjs) carrying a
  ...
  // Fail loudly, matching src/scripts/build-cv-html.mjs and build-cv-latex.mjs. Shipping a
...
async function main() {
  ...
  if (args.help || !args.payload) {
    console.log(`
Usage:
  node src/scripts/generate-cover-letter.mjs --payload payload.json [--out output/path.pdf] [--format letter|a4] [--report NNN]
