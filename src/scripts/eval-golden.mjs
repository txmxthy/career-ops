#!/usr/bin/env node
/**
 * src/scripts/eval-golden.mjs — golden-set eval harness for cheap-model routing (#1354)
 *
 ...
 * recorded fixture for $0 deterministic CI, or live via src/scripts/openai-eval.mjs),
 ...
 *
 * Usage:
 *   node src/scripts/eval-golden.mjs --replay --model cheap-stub     # offline, deterministic ($0)
 *   node src/scripts/eval-golden.mjs --live   --model gpt-4o-mini    # calls src/scripts/openai-eval.mjs (needs key + cv.md)
...
if (args.includes('--help') || args.includes('-h')) {
  console.log(`src/scripts/eval-golden.mjs — golden-set eval harness (#1354)

  --replay         Replay recorded fixtures (default; offline, $0, deterministic)
  --live           Call the model live via src/scripts/openai-eval.mjs (needs key + cv.md)
 ...
 * deterministic; in live mode it shells out to src/scripts/openai-eval.mjs, reusing the
...
function getCompletion(testCase) {
  ...
  try {
    ...
    const res = spawnSync(process.execPath,
      [join(ROOT, 'src/scripts/openai-eval.mjs'), '--file', jdFile, '--model', model, '--no-save'],
      { encoding: 'utf8', env: process.env, timeout: 360000 });
    if (res.status !== 0) {
      throw new Error(`src/scripts/openai-eval.mjs exited ${res.status}: ${(res.stderr || '').slice(0, 200)}`);
    }
    return res.stdout || '';
  } finally {
