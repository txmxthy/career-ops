#!/usr/bin/env node
// src/scripts/intake.mjs — deterministic half of the `intake` mode (#1723).
//
...
//
// Division of labor (mirrors modes/add.md + src/scripts/add-entry.mjs):
...
// src/scripts/generate-latex.mjs engine ladder): born-digital PDFs — the dominant case
...
//
// Usage:
//   node src/scripts/intake.mjs                 # scan + extract, JSON to stdout
//   node src/scripts/intake.mjs --summary       # human-readable table instead of JSON
//   node src/scripts/intake.mjs --text <path>   # full extracted text of one source
//   node src/scripts/intake.mjs --commit <path …>
//                                   # record the confirmed sources as ingested
//                                   # (run only after the user confirmed)
//   node src/scripts/intake.mjs --commit --all  # record every source with new material —
//                                   # only when the user merged all of them
//   node src/scripts/intake.mjs --self-test     # pure-function self-test, no filesystem
...
function main() {
  ...
  if (hasFlag(args, '--text')) {
    ...
    if (!target) { console.error('Usage: node src/scripts/intake.mjs --text <path relative to documents/>'); process.exit(1); }
  ...
  if (commitIdx !== -1) {
    ...
    if (!all && only.length === 0) {
      console.error(
        'Nothing to commit. Pass the confirmed sources — node src/scripts/intake.mjs --commit <path> [<path> …] — '
        + 'or node src/scripts/intake.mjs --commit --all when every source with new material was merged. '
