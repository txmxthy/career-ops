#!/usr/bin/env node
/**
 * src/scripts/funnel-velocity.mjs — Funnel calibration vs market benchmarks + stage velocity
 *
 * Three payloads, decreasing availability:
 *   1. calibration — own funnel rates (canonical ever* definition imported from
 *      src/scripts/stats.mjs) vs candidate-side market benchmark ranges. Works day one.
 ...
 *
 * Run: node src/scripts/funnel-velocity.mjs             (JSON)
...
const USAGE = `Usage:
  node src/scripts/funnel-velocity.mjs                       # JSON report
  node src/scripts/funnel-velocity.mjs --summary             # human-readable report
  node src/scripts/funnel-velocity.mjs --benchmarks <path>   # override the benchmark YAML file
  node src/scripts/funnel-velocity.mjs --self-test           # run the built-in fixtures
  node src/scripts/funnel-velocity.mjs --help|-h              # show this message`;
