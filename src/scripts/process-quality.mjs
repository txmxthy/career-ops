#!/usr/bin/env node
/**
 * src/scripts/process-quality.mjs — Recruiting-Process Friction Aggregator for career-ops
 *
 ...
 *
 * Run: node src/scripts/process-quality.mjs             (JSON to stdout)
 *      node src/scripts/process-quality.mjs --summary   (human-readable table)
 *      node src/scripts/process-quality.mjs --min-threshold 2  (min total interviews per company to report)
 *      node src/scripts/process-quality.mjs --file path/to/active-interviews.md  (override the data path; test isolation)
 *      node src/scripts/process-quality.mjs --self-test
 *
...
// --file overrides the data path — mirrors src/scripts/validate-portals.mjs / verify-portals.mjs's
...
export function parseActiveInterviews(content) {
  ...
  // column names: src/scripts/rejection-latency.mjs:172 and findColumn() below both look
...
const USAGE = `Usage:
  node src/scripts/process-quality.mjs                        # JSON report
  node src/scripts/process-quality.mjs --summary              # human-readable table
  node src/scripts/process-quality.mjs --file <path>          # a different active-interviews.md
  node src/scripts/process-quality.mjs --min-threshold <N>    # minimum rounds before a company is reported (default 1)
  node src/scripts/process-quality.mjs --self-test            # run the built-in fixtures
  node src/scripts/process-quality.mjs --help                 # show this message`;

if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  // Inside the main-module guard, not at import time: src/scripts/rejection-latency.mjs
