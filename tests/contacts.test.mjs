/**
 * tests/contacts.test.mjs — Systematic test suite for src/scripts/contacts.mjs
 *
 ...
 *
 * Run: node tests/contacts.test.mjs
 */

import { parseContacts, escapeVcard, foldLine, slug, uidPart, normalizeForHash, contactUid, contactToVcard, buildVcf } from '../src/scripts/contacts.mjs';
...
// src/scripts/contacts.mjs resolves its paths from import.meta.url and is zero-dep, so a
...
try {
  ...
  const emptyVcfOut = execFileSync('node', [join(emptyRoot, 'src/scripts/contacts.mjs'), '--vcf'], { encoding: 'utf-8', timeout: 10000 });
