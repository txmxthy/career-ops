 ...
 *
 * merge-tracker.mjs and src/scripts/verify-pipeline.mjs used to parse applications.md by
...
// release-please component; see src/scripts/validate-system-paths-coverage.mjs
...
function makeSandbox(trackerContent, additions = {}) {
  ...
  // in tests/run-all.mjs (see PATCHES.md patch 10).
...
// ── Test 2: verify-pipeline is clean on a 10-column tracker ────────────────
{
  ...
  const res = runScript('src/scripts/verify-pipeline.mjs', [], sb);
...
// ── Test 3: legacy 9-column layout still works (back-compat) ───────────────
{
  ...
  const verify = runScript('src/scripts/verify-pipeline.mjs', [], sb);
  const row = dataRows(sb.tracker).find(l => l.includes('Globex'));
  const cells = row ? row.split('|').map(s => s.trim()) : [];
...
// src/scripts/normalize-statuses.mjs read Status at parts[6], Score at parts[5] and Notes
...
`;
  ...
  const res = runScript('src/scripts/normalize-statuses.mjs', [], sb);
...
`;
  ...
  const norm = runScript('src/scripts/normalize-statuses.mjs', [], sb);
...
`;
  ...
  const res = runScript('src/scripts/dedup-tracker.mjs', [], sb);
  const rows = dataRows(sb.tracker);
