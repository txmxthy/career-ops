...
//
// Covers the deterministic half (src/scripts/intake.mjs): source classification, the
...
const intake = await import(pathToFileURL(join(ROOT, 'src/scripts/intake.mjs')).href);
...
// ── CLI round-trip on an isolated temp documents/ ────────────────────────
{
  ...
  try {
    const scan1 = JSON.parse(run(NODE, ['src/scripts/intake.mjs'], { env }) || 'null');
    ...
    const refused = run(NODE, ['src/scripts/intake.mjs', '--commit', '--summary'], { env });
    const refusedErr = (lastRunFailure() || {}).stderr || '';
    const afterRefusal = JSON.parse(run(NODE, ['src/scripts/intake.mjs'], { env }) || 'null');
    ...
    run(NODE, ['src/scripts/intake.mjs', '--commit', '--all'], { env });
    const scan2 = JSON.parse(run(NODE, ['src/scripts/intake.mjs'], { env }) || 'null');
    ...
    writeFileSync(join(docsDir, 'cv', 'master.md'), '# CV\n\n- Built things\n- Shipped more\n');
    const scan3 = JSON.parse(run(NODE, ['src/scripts/intake.mjs'], { env }) || 'null');
    ...
    const text = run(NODE, ['src/scripts/intake.mjs', '--text', 'cv/master.md'], { env });
    ...
    run(NODE, ['src/scripts/intake.mjs', '--commit', 'cv/master.md'], { env });
    const scan4 = JSON.parse(run(NODE, ['src/scripts/intake.mjs'], { env }) || 'null');
    ...
    const escaped = run(NODE, ['src/scripts/intake.mjs', '--text', '../intake-state.json'], { env });
    ...
    const unreadable = run(NODE, ['src/scripts/intake.mjs', '--text', 'cv/notes.md'], { env });
    ...
    const selfTest = run(NODE, ['src/scripts/intake.mjs', '--self-test'], { env });
    if (selfTest !== null && selfTest.includes('0 failed')) pass('src/scripts/intake.mjs --self-test passes');
    else fail('src/scripts/intake.mjs --self-test failed');
  } finally {
...
// the plugin-manifest traversal checks in tests/run-all.mjs already do — CI runs
// elevated and still exercises every one of them, so nothing is lost there.
{
  ...
  try {
    ...
    const scan = JSON.parse(run(NODE, ['src/scripts/intake.mjs'], { env }) || 'null');
    ...
    const text = run(NODE, ['src/scripts/intake.mjs', '--text', 'cv/linked.md'], { env });
    ...
    const aliased = JSON.parse(run(NODE, ['src/scripts/intake.mjs'], { env }) || 'null');
    ...
    const nested = JSON.parse(run(NODE, ['src/scripts/intake.mjs'], { env }) || 'null');
