...
// Run src/scripts/agent-inbox.mjs against a provisioned queue file; returns stdout.
...
console.log('7. concurrent adds do not lose items (append, not rewrite)');
{
  ...
  for (const l of losers) {
    ...
    // Generous, because the owner record src/lib/pipeline-lock.mjs appends to a
...
import { withPipelineLock } from ${JSON.stringify(pathToFileURL(join(ROOT, 'src/lib/pipeline-lock.mjs')).href)};
