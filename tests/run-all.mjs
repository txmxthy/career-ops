...
/**
 * tests/run-all.mjs — Comprehensive test suite for career-ops
 *
 * Run before merging any PR or pushing changes.
 * Tests: syntax, scripts, dashboard, data contract, personal data, paths.
 *
 * Usage:
 *   node tests/run-all.mjs                        # Run all tests
 *   node tests/run-all.mjs --quick                # Skip dashboard build (faster)
 *   node tests/run-all.mjs --only <substring>      # Run ONLY discovered tests/**\/*.test.mjs
...
import { pass, fail, warn, run, lastRunFailure, formatRunFailure, fileExists, finish, ROOT, QUICK, NODE, DEFAULT_SCRIPT_TIMEOUT_MS, getBash, toBashPath, hermeticGitEnv } from './helpers.mjs';
...
async function runDiscovered(filter = null) {
  ...
  for (const f of files) {
    ...
    if (/\bfinish\s*\(\s*\)/.test(src)) {
      fail(`${f.slice(ROOT.length + 1)} calls finish() — only tests/run-all.mjs may print the global summary; discovered suites use pass/fail and return`);
      continue;
    }
...
const scripts = [
  { name: 'src/scripts/cv-sync-check.mjs', expectExit: 1, allowFail: true }, // fails without cv.md (normal in repo)
  { name: 'src/scripts/verify-pipeline.mjs', expectExit: 0 },
  ...
  { name: 'src/scripts/normalize-statuses.mjs --dry-run', expectExit: 0 },
  { name: 'src/scripts/dedup-tracker.mjs --dry-run', expectExit: 0 },
  { name: 'merge-tracker.mjs --dry-run', expectExit: 0 },
  { name: 'src/scripts/reconcile-pipeline.mjs --dry-run', expectExit: 0 },
  { name: 'src/scripts/analyze-patterns.mjs --self-test', expectExit: 0 },
  { name: 'src/scripts/check-table-freshness.mjs --self-test', expectExit: 0 },
  { name: 'src/scripts/upskill.mjs --self-test', expectExit: 0 },
  { name: 'src/scripts/detect-reposts.mjs --self-test', expectExit: 0 },
  { name: 'src/scripts/rank-pipeline.mjs --self-test', expectExit: 0 },
  { name: 'src/scripts/discover-ats.mjs --self-test', expectExit: 0 },
  { name: 'src/scripts/process-quality.mjs --self-test', expectExit: 0 },
  { name: 'src/scripts/company-history.mjs --self-test', expectExit: 0 },
  { name: 'src/scripts/rejection-latency.mjs --self-test', expectExit: 0 },
  { name: 'src/scripts/salary-gap.mjs --self-test', expectExit: 0 },
  { name: 'src/scripts/negotiation-roi.mjs --self-test', expectExit: 0 },
  { name: 'src/scripts/funnel-velocity.mjs --self-test', expectExit: 0 },
  { name: 'src/scripts/img-to-pdf.mjs --self-test', expectExit: 0 },
  { name: 'src/scripts/assessment-log.mjs --self-test', expectExit: 0 },
  { name: 'src/scripts/weekly-digest.mjs --self-test', expectExit: 0 },
  { name: 'src/scripts/build-cv-html.mjs --test', expectExit: 0 },
  { name: 'src/scripts/jd-skill-gap.mjs --self-test', expectExit: 0 },
  { name: 'src/scripts/story-provenance-check.mjs --self-test', expectExit: 0 },
  { name: 'src/scripts/verify-cv-facts.mjs --self-test', expectExit: 0 },
  { name: 'src/scripts/contacts.mjs --self-test', expectExit: 0 },
  { name: 'src/scripts/company-funded.mjs --self-test', expectExit: 0 },
  { name: 'src/scripts/invite-match.mjs --self-test', expectExit: 0 },
  { name: 'tests/invite-match.test.mjs', expectExit: 0 },
  { name: 'src/scripts/tracker-sync-check.mjs --self-test', expectExit: 0 },
  { name: 'tests/updater-migration.test.mjs', expectExit: 0 },
  { name: 'tests/tracker-columns.test.mjs', expectExit: 0 },
  { name: 'tests/agent-inbox.test.mjs', expectExit: 0 },
  { name: 'tests/followup-seed.test.mjs', expectExit: 0 },
  { name: 'tests/paste-reply.test.mjs', expectExit: 0 },
  { name: 'tests/set-status.test.mjs', expectExit: 0 },
  ...
  { name: 'tests/tracker-writer-lock.test.mjs', expectExit: 0, timeoutMs: 180_000 },
  ...
  { name: 'tests/trust-validator.test.mjs', expectExit: 0 },
  { name: 'tests/salary-filter.test.mjs', expectExit: 0 },
  { name: 'tests/detect-reposts.test.mjs', expectExit: 0 },
  { name: 'tests/discover-ats.test.mjs', expectExit: 0 },
  { name: 'tests/followup-cadence.test.mjs', expectExit: 0 },
  { name: 'tests/process-quality.test.mjs', expectExit: 0 },
  { name: 'tests/company-history.test.mjs', expectExit: 0 },
  { name: 'tests/contacts.test.mjs', expectExit: 0 },
  { name: 'tests/reply-matcher.test.mjs', expectExit: 0 },
  { name: 'src/scripts/validate-portals.mjs --file templates/portals.example.yml', expectExit: 0 },
  { name: 'src/scripts/validate-system-paths-coverage.mjs --self-test', expectExit: 0 },
  ...
  { name: 'src/scripts/validate-untrusted-content-coverage.mjs --self-test', expectExit: 0 },
  ...
  { name: 'src/lib/seed-fixture.mjs --self-test', expectExit: 0 },
  { name: 'src/scripts/archive-posting.mjs --help', expectExit: 0 },
];
...
try {
  ...
  for (const script of scripts) {
    ...
    // tests/tracker-writer-lock.test.mjs was killed rather than flagged is that
  ...
  // src/scripts/assessment-log.mjs CLI contract (#2797): help aliases print one shared
  ...
  {
    const assessmentCli = (...argv) => spawnSync(NODE, [join(scriptTmp, 'src/scripts/assessment-log.mjs'), ...argv], {
      cwd: scriptTmp,
      encoding: 'utf-8',
      timeout: 30000,
      stdio: ['pipe', 'pipe', 'pipe'],
    ...
    if (helpR.status === 0 && hR.status === 0 && helpR.stdout.includes('Usage:')
      ...
      pass('src/scripts/assessment-log.mjs --help/-h print the shared usage block and exit 0 (#2797)');
    } else {
      fail(`src/scripts/assessment-log.mjs help handling broken: ${JSON.stringify({ help: { status: helpR.status, stdout: helpR.stdout, stderr: helpR.stderr }, h: { status: hR.status, stdout: hR.stdout, stderr: hR.stderr } })}`);
    }
    ...
    if (typoR.status === 1 && typoR.stderr.includes('unrecognized flag')
      ...
      pass('src/scripts/assessment-log.mjs rejects and names an unrecognized leading-dash flag (#2797)');
    } else {
      fail(`src/scripts/assessment-log.mjs unknown flag handling broken: ${JSON.stringify({ typo: { status: typoR.status, stdout: typoR.stdout, stderr: typoR.stderr }, misplacedAddFlag: { status: misplacedAddFlagR.status, stdout: misplacedAddFlagR.stdout, stderr: misplacedAddFlagR.stderr } })}`);
    }
    ...
    if (addR.status === 0 && added?.added === true
      ...
      pass('src/scripts/assessment-log.mjs preserves add/summary flags and dash-containing values (#2797 regression)');
    } else {
      fail(`src/scripts/assessment-log.mjs existing CLI behavior regressed: ${JSON.stringify({ add: { status: addR.status, stdout: addR.stdout, stderr: addR.stderr }, summary: { status: summaryR.status, stdout: summaryR.stdout, stderr: summaryR.stderr } })}`);
    }
  }

  // src/scripts/reply-watch.mjs CLI flag validation (#2743). main() used to read
  ...
  {
    const replyWatchCli = (...argv) => spawnSync(NODE, [join(scriptTmp, 'src/scripts/reply-watch.mjs'), ...argv], {
      cwd: scriptTmp,
      encoding: 'utf-8',
      timeout: 30000,
      stdio: ['pipe', 'pipe', 'pipe'],
    ...
    if (helpOk && hOk && bogusOk) {
      pass('src/scripts/reply-watch.mjs rejects --help/-h/unrecognized flags without creating a stray candidates file (#2743)');
    } else {
      fail(`src/scripts/reply-watch.mjs flag validation broken: help=${JSON.stringify({ status: helpR.status, stdout: helpR.stdout, exists: existsSync(join(scriptTmp, '--help')) })} h=${JSON.stringify({ status: hR.status, stdout: hR.stdout, exists: existsSync(join(scriptTmp, '-h')) })} bogus=${JSON.stringify({ status: bogusR.status, stderr: bogusR.stderr, exists: existsSync(join(scriptTmp, '--bogus')) })}`);
    }
    ...
    // otherwise `src/scripts/reply-watch.mjs --help --bogus` would exit 0 instead of erroring.
    ...
    if (mixedOk) {
      pass('src/scripts/reply-watch.mjs rejects an unrecognized flag even when --help is also present (#2745)');
    } else {
      fail(`src/scripts/reply-watch.mjs --help+--bogus should still error, not exit clean: ${JSON.stringify({ status: mixedR.status, stdout: mixedR.stdout, stderr: mixedR.stderr })}`);
    }
    ...
    if (noArgsOk && explicitOk) {
      pass('src/scripts/reply-watch.mjs no-args-uses-default and explicit-path behaviors unchanged (#2743 regression)');
    } else {
      fail(`src/scripts/reply-watch.mjs regression broken: noArgs=${JSON.stringify({ status: noArgsR.status, exists: existsSync(defaultCandidatesFile) })} explicit=${JSON.stringify({ status: explicitR.status, exists: existsSync(explicitPath) })}`);
    }
  }
} finally {
...
try {
  ...
  const hiddenResult = run(NODE, ['src/scripts/verify-cv-facts.mjs', hiddenScriptMetric], {
    stdio: ['pipe', 'pipe', 'pipe'],
  });
  ...
  const visibleResult = run(NODE, ['src/scripts/verify-cv-facts.mjs', visibleMetric], {
    stdio: ['pipe', 'pipe', 'pipe'],
  });
...
try {
  const { classifyLiveness } = await import(pathToFileURL(join(ROOT, 'src/lib/liveness-core.mjs')).href);
  ...
  // Liveness API rung (src/lib/liveness-api.mjs) — the zero-token ATS first rung. We test the
  ...
  const { resolveAtsApi, classifyAshbyBoard, checkLivenessViaApi } = await import(pathToFileURL(join(ROOT, 'src/lib/liveness-api.mjs')).href);
  ...
  // Headed-fallback-on-challenge path (src/lib/liveness-browser.mjs). Fake Playwright
  ...
  const { checkUrlLiveness, checkUrlLivenessWithFallback, isChallengeResult, jitteredDelayMs } =
    await import(pathToFileURL(join(ROOT, 'src/lib/liveness-browser.mjs')).href);
  ...
  const { rejectPrivateOrInvalid, setHostResolver } = await import(
    pathToFileURL(join(ROOT, 'src/lib/liveness-browser.mjs')).href
  );
  const blockCases = [
...
//
// src/scripts/archive-posting.mjs drives Playwright at a URL the user pastes in (or that
...
// two-layer guard src/lib/liveness-browser.mjs exports rather than carrying a second
...
try {
  const { archiveUrl, installEgressGuard } = await import(
    pathToFileURL(join(ROOT, 'src/scripts/archive-posting.mjs')).href
  );
...
{
  const cov = spawnSync(process.execPath, [join(ROOT, 'src/scripts/validate-system-paths-coverage.mjs')], {
    cwd: ROOT,
    encoding: 'utf-8',
  });
  if (cov.status === 0) {
...
{
  const untrusted = spawnSync(process.execPath, [join(ROOT, 'src/scripts/validate-untrusted-content-coverage.mjs')], {
    cwd: ROOT,
    encoding: 'utf-8',
  });
...
const allowedFiles = [
  ...
  'package.json', '.github/FUNDING.yml', 'CLAUDE.md', 'AGENTS.md', 'go.mod', 'tests/run-all.mjs',
...
const ABS_PATH_EXCLUDE = ['README.md', 'LICENSE', 'CLAUDE.md', 'tests/run-all.mjs'];
...
const patternsMachineFields = readFile('src/scripts/analyze-patterns.mjs').match(/const MACHINE_SUMMARY_FIELDS = new Set\(\[([\s\S]*?)\]\);/)?.[1] ?? '';
if (
...
if (
  ...
  applyMode.includes('src/scripts/application-answers.mjs')
) {
...
if (
  run(NODE, ['src/scripts/application-answers.mjs', '--report', '--input'], { stdio: ['pipe', 'pipe', 'pipe'] }) === null &&
  run(NODE, ['src/scripts/application-answers.mjs', '--report', '--input', 'answers.json'], { stdio: ['pipe', 'pipe', 'pipe'] }) === null
) {
...
// --- Block G AI-screening disclosure signal (#2892, jurisdiction-compliance-lens umbrella #2026) ---
{
  ...
  // 9. src/scripts/check-table-freshness.mjs discovers the new table automatically —
  ...
  try {
    const freshnessMod = await import('../src/scripts/check-table-freshness.mjs');
    ...
    if (allSeedsDiscovered && allRowsHaveAsOf) {
      pass(`src/scripts/check-table-freshness.mjs extractRows() auto-discovers all 3 seed rows (US-NY-NYC, US-IL, EU) of jurisdiction-ai-screening-disclosure.yml with as_of present, tolerating future community-contributed rows (${rows.length} rows discovered) — no registration step needed (#2036, #2896 CodeRabbit finding 3)`);
    } else {
      fail(`src/scripts/check-table-freshness.mjs extractRows() did not cleanly discover the seed rows of jurisdiction-ai-screening-disclosure.yml — discovered keys: ${[...discoveredKeys].join(', ') || '(none)'}, missing seeds: ${seedKeys.filter((k) => !discoveredKeys.has(k)).join(', ') || 'none'}, any missing as_of: ${!allRowsHaveAsOf} (#2896)`);
    }
  } catch (e) {
    fail(`could not import src/scripts/check-table-freshness.mjs to verify auto-discovery: ${e.message} (#2892)`);
  }
}
...
if (
  ...
  pipelineMode.includes('src/scripts/check-liveness.mjs') &&
...
if (
  ...
  trackerModeDoc.includes('src/scripts/salary-gap.mjs')
) {
...
if (patternsModeDoc.includes('src/scripts/salary-gap.mjs')) {
...
if (
  ...
  upskillModeDoc.includes('src/lib/liveness-core.mjs') &&
...
// --- src/scripts/company-history.mjs wiring across mode docs (Task 6) ---
const followupModeDoc = readFile('modes/followup.md');

if (
  ofertaMode.includes('src/scripts/company-history.mjs') &&
  ofertaMode.includes('Prior-contact FYI') &&
  ofertaMode.includes('Not a legitimacy signal')
) {
  pass('oferta mode wires src/scripts/company-history.mjs and keeps the prior-contact FYI out of the legitimacy tier');
} else {
  fail('oferta mode missing src/scripts/company-history.mjs reference, the "Prior-contact FYI" block, or the "Not a legitimacy signal" guardrail');
}
...
if (
  patternsModeDoc.includes('src/scripts/company-history.mjs') &&
  ...
  fail('patterns mode missing src/scripts/company-history.mjs lens, the "Hygiene first, always." cue, aged-Applied mention, or hygiene-before-aged-Applied ordering');
}

if (followupModeDoc.includes('src/scripts/company-history.mjs') && followupModeDoc.includes('silent-on-you')) {
  pass('followup mode references both src/scripts/company-history.mjs and the silent-on-you label when setting expectations');
} else {
  fail('followup mode must reference BOTH src/scripts/company-history.mjs and silent-on-you');
}

if (trackerModeDoc.includes('src/scripts/company-history.mjs') && trackerModeDoc.includes('silent-on-you')) {
  pass('tracker mode offers src/scripts/company-history.mjs when a silent-on-you company is present');
} else {
  fail('tracker mode missing src/scripts/company-history.mjs reference or the silent-on-you trigger');
}
...
// src/scripts/company-history.mjs — every legitimacy Source must be observable without
...
// script output.
if (
  patternsModeDoc.includes('src/scripts/funnel-velocity.mjs') &&
...
if (
  trackerModeDoc.includes('src/scripts/funnel-velocity.mjs') &&
...
if (followupModeDoc.includes('src/scripts/funnel-velocity.mjs') && followupModeDoc.includes('--on')) {
...
// --- contacts phonebook wiring (src/scripts/contacts.mjs <-> contacto mode) ---
const contactoModeDoc = readFile('modes/contacto.md');

if (
  contactoModeDoc.includes('data/contacts.tsv') &&
  contactoModeDoc.includes('src/scripts/contacts.mjs --vcf') &&
  ...
  fail('contacto missing the save-to-contacts.tsv step, the no-auto-save rule, or the src/scripts/contacts.mjs --vcf mention');
}
...
try {
  const { appendToPipeline } = await import(pathToFileURL(join(ROOT, 'scan.mjs')).href);
  const { acquirePipelineLock, LockTimeoutError } = await import(pathToFileURL(join(ROOT, 'src/lib/pipeline-lock.mjs')).href);
  ...
  try {
    ...
    try {
      ...
      if (e instanceof LockTimeoutError) pass('appendToPipeline() shares src/lib/pipeline-lock.mjs — correctly blocked on a lock held elsewhere (LockTimeoutError)');
  ...
  fail(`src/lib/pipeline-lock.mjs sharing test crashed: ${err.message}`);
}
...
// path must offer `src/scripts/browser-extract.mjs` behind `scan.extractor`, with a silent
// MCP fallback — so the flag actually reaches the JD paths, not just scan/pipeline.
{
  ...
  const missing = jdPathModes.filter((m) => {
    ...
    return !(src.includes('src/scripts/browser-extract.mjs') && src.includes('scan.extractor'));
  });
  if (missing.length === 0) {
  ...
  if (!readFile('modes/apply.md').includes('src/scripts/browser-extract.mjs')) {
    pass('apply mode does not route through the read-only extractor (#1449)');
  } else {
    fail('apply mode references src/scripts/browser-extract.mjs — the extractor must not touch the apply/form path');
  }
...
try {
  const nodeTrackerWriters = [
    ['src/scripts/dedup-tracker.mjs', 1],
    ['src/scripts/normalize-statuses.mjs', 1],
    ['src/scripts/reply-watch.mjs', 1],
    ['tracker.mjs', 2],
  ];
...
`, 'utf-8');
  ...
  const validResult = run(NODE, ['src/scripts/validate-portals.mjs', '--file', validPath]);
  ...
  const validProviderPluginResult = run(NODE, ['src/scripts/validate-portals.mjs', '--file', validProviderPluginPath]);
  ...
  const exampleResult = run(NODE, ['src/scripts/validate-portals.mjs', '--file', 'templates/portals.example.yml']);
  ...
  const invalidProviderResult = run(NODE, ['src/scripts/validate-portals.mjs', '--file', invalidProviderPath]);
  ...
  const emptyKeywordResult = run(NODE, ['src/scripts/validate-portals.mjs', '--file', emptyKeywordPath]);
  ...
  const duplicateCompanyResult = run(NODE, ['src/scripts/validate-portals.mjs', '--file', duplicateCompanyPath]);
  ...
  const badContentFilterResult = run(NODE, ['src/scripts/validate-portals.mjs', '--file', badContentFilterPath]);
  ...
  const deadByTitleKeywordResult = run(NODE, ['src/scripts/validate-portals.mjs', '--file', deadByTitleKeywordPath]);
  ...
  const badVisaFilterResult = run(NODE, ['src/scripts/validate-portals.mjs', '--file', badVisaFilterPath]);
...
// ── 10c. SLUG AUTO-FIXER (src/scripts/fix-slugs.mjs) ─────────────────────────

console.log('\n10c. Slug auto-fixer');

try {
  const { splitCompanyBlocks, computeFixes } = await import(
    pathToFileURL(join(ROOT, 'src/scripts/fix-slugs.mjs')).href
  );
  ...
  try {
    ...
    // nothing but 15 s.
    execFileSync(NODE, [join(ROOT, 'src/scripts/fix-slugs.mjs'), '--file', dryRunPortals, '--dry-run'], {
      cwd: ROOT,
      timeout: 2000,
    });
  } catch {
  ...
  if (afterDryRun === beforeDryRun) {
    pass('src/scripts/fix-slugs.mjs --dry-run (default) never writes to portals.yml');
  } else {
    fail('src/scripts/fix-slugs.mjs --dry-run wrote to portals.yml — must require --fix/--apply');
  }
  rmSync(dryRunTmp, { recursive: true, force: true });

  // CLI flag validation (#2980)
  const helpOut = spawnSync(NODE, [join(ROOT, 'src/scripts/fix-slugs.mjs'), '--help'], { encoding: 'utf-8' });
  const hOut = spawnSync(NODE, [join(ROOT, 'src/scripts/fix-slugs.mjs'), '-h'], { encoding: 'utf-8' });
  if (helpOut.status === 0 && hOut.status === 0 &&
      helpOut.stdout.includes('Usage:') && helpOut.stdout.includes('src/scripts/fix-slugs.mjs') &&
      hOut.stdout === helpOut.stdout) {
    pass('src/scripts/fix-slugs.mjs --help/-h print usage and exit 0');
  } else {
    fail(`src/scripts/fix-slugs.mjs --help/-h failed => status=${helpOut.status}/${hOut.status}`);
  }

  const badFlagOut = spawnSync(NODE, [join(ROOT, 'src/scripts/fix-slugs.mjs'), '--dryrun'], { encoding: 'utf-8' });
  if (badFlagOut.status === 1 && badFlagOut.stderr.includes('unrecognized flag(s): --dryrun') && badFlagOut.stderr.includes('Valid flags:')) {
    pass('src/scripts/fix-slugs.mjs rejects unrecognized flags with exit 1');
  } else {
    fail(`src/scripts/fix-slugs.mjs bad flag handling => status=${badFlagOut.status} stderr=${badFlagOut.stderr}`);
  }

  const missingFileOut = spawnSync(NODE, [join(ROOT, 'src/scripts/fix-slugs.mjs'), '--file'], { encoding: 'utf-8' });
  const fileFlagNextOut = spawnSync(NODE, [join(ROOT, 'src/scripts/fix-slugs.mjs'), '--file', '--fix'], { encoding: 'utf-8' });
  if (missingFileOut.status === 1 && missingFileOut.stderr.includes('--file requires a value') &&
    ...
    pass('src/scripts/fix-slugs.mjs rejects missing --file value with exit 1');
  } else {
    fail(`src/scripts/fix-slugs.mjs missing --file value handling => bare: ${missingFileOut.status} nextFlag: ${fileFlagNextOut.status}`);
  }
} catch (e) {
...
console.log('\n12. src/scripts/archive-posting.mjs');
...
for (const [url, expected] of [
  ...
  const out = run(NODE, ['src/scripts/archive-posting.mjs', '--dry-run', url]);
...
const overrideOut = run(NODE, [
  'src/scripts/archive-posting.mjs', '--dry-run',
...
const refOut = run(NODE, ['src/scripts/archive-posting.mjs', '--dry-run', 'https://boards.greenhouse.io/openai/jobs/123']);
...
run(NODE, ['src/scripts/archive-posting.mjs']) !== null
...
run(NODE, ['src/scripts/archive-posting.mjs', '--dry-run', 'https://boards.greenhouse.io/openai/jobs/123', '--report']) === null
...
for (const argv of [['--report', '4'], ['--report=4']]) {
  const keyed = run(NODE, ['src/scripts/archive-posting.mjs', '--dry-run', 'https://boards.greenhouse.io/openai/jobs/123', ...argv]);
...
run(NODE, ['src/scripts/archive-posting.mjs', '--dry-run']) === null
...
run(NODE, ['src/scripts/archive-posting.mjs', '--company=Acme']) === null
...
const reportEqOut = run(NODE, ['src/scripts/archive-posting.mjs', '--dry-run', '--report=42', 'https://boards.greenhouse.io/openai/jobs/123']);
...
const reportSpaceOut = run(NODE, ['src/scripts/archive-posting.mjs', '--dry-run', '--report', '42', 'https://boards.greenhouse.io/openai/jobs/123']);
...
const noReportOut = run(NODE, ['src/scripts/archive-posting.mjs', '--dry-run', 'https://boards.greenhouse.io/openai/jobs/123']);
noReportOut?.includes(`jds/${todayStr}_`)
  ? pass('no --report: filename shape unchanged')
  : fail('no --report: filename shape regressed');

run(NODE, ['src/scripts/archive-posting.mjs', '--dry-run', '--report=abc', 'https://boards.greenhouse.io/openai/jobs/123']) === null
  ? pass('--report with non-numeric value: exits non-zero')
  : fail('--report with non-numeric value: should be rejected, not ignored');

run(NODE, ['src/scripts/archive-posting.mjs', '--pipeline', '--report=42']) === null
...
if (!hasBrowser) {
  ...
  if (!liveJobUrl) {
    ...
    const startedAt = Date.now();
    const archiveOut = run('node', ['src/scripts/archive-posting.mjs', liveJobUrl], { timeout: 60000 });
...
try {
  ...
  try {
    ...
    // (scan.mjs, src/lib/cv-templates.mjs, providers/_profile-keywords.mjs, plugins/_engine.mjs)
...
console.log('\n14b. src/scripts/add-entry.mjs (dedup + insertion)');

try {
  const addMod = await import(pathToFileURL(join(ROOT, 'src/scripts/add-entry.mjs')).href);
  ...
  try {
    ...
    const helpOut = spawnSync(NODE, [join(ROOT, 'src/scripts/add-entry.mjs'), '--help'], { env, encoding: 'utf-8' });
    const hOut = spawnSync(NODE, [join(ROOT, 'src/scripts/add-entry.mjs'), '-h'], { env, encoding: 'utf-8' });
    if (helpOut.status === 0 && hOut.status === 0 &&
    ...
    const badFlag = spawnSync(NODE, [join(ROOT, 'src/scripts/add-entry.mjs'), missingPayloadPath, '--sumary'], { env, encoding: 'utf-8' });
    ...
    const stdinDryRun = spawnSync(NODE, [join(ROOT, 'src/scripts/add-entry.mjs'), '--stdin', '--dry-run'], {
      env,
      encoding: 'utf-8',
    ...
    execFileSync(NODE, [join(ROOT, 'src/scripts/add-entry.mjs'), payloadPath, '--dry-run'], { env, encoding: 'utf-8' });
    ...
    const realOut = JSON.parse(execFileSync(NODE, [join(ROOT, 'src/scripts/add-entry.mjs'), payloadPath], { env, encoding: 'utf-8' }));
    ...
    const rerun = JSON.parse(execFileSync(NODE, [join(ROOT, 'src/scripts/add-entry.mjs'), payloadPath], { env, encoding: 'utf-8' }));
  ...
  // each other, so an entry could land under the wrong section.
  {
    ...
    try {
      ...
      const out = JSON.parse(execFileSync(NODE, [join(ROOT, 'src/scripts/add-entry.mjs'), payloadPath], { env, encoding: 'utf-8' }));
      out.cv.status === 'added'
      ...
      execFileSync(NODE, [join(ROOT, 'src/scripts/add-entry.mjs'), p2], { env, encoding: 'utf-8' });
...
try {
  const { normalizeReportLink } = await import(pathToFileURL(join(ROOT, 'src/scripts/tracker-links.mjs')).href);
  const repo = '/repo';
...
try {
  ...
  const evaluatorSources = ['src/scripts/ollama-eval.mjs', 'src/scripts/openai-eval.mjs', 'src/scripts/gemini-eval.mjs', 'src/scripts/openrouter-runner.mjs']
...
try {
  ...
  try {
    ...
    const vpOut = run(NODE, ['src/scripts/verify-pipeline.mjs'], { env: vpEnv, stdio: ['pipe', 'pipe', 'pipe'] });
    if (vpOut === null) {
    ...
    const vpClean = run(NODE, ['src/scripts/verify-pipeline.mjs'], { env: vpEnv, stdio: ['pipe', 'pipe', 'pipe'] });
    if (vpClean !== null &&
...
try {
  ...
  try {
    ...
    const tkOut = run(NODE, ['src/scripts/verify-pipeline.mjs'], { env: tkEnv, stdio: ['pipe', 'pipe', 'pipe'] });
    if (tkOut === null) {
    ...
    const tkCtl = run(NODE, ['src/scripts/verify-pipeline.mjs'], { env: tkEnv, stdio: ['pipe', 'pipe', 'pipe'] });
...
try {
  ...
  try {
    ...
    const orOut = run(NODE, ['src/scripts/verify-pipeline.mjs'], { env: orEnv, stdio: ['pipe', 'pipe', 'pipe'] });
    if (orOut === null) {
...
try {
  ...
  try {
    ...
    let dupNumOut;
    try {
      dupNumOut = execFileSync(NODE, ['src/scripts/verify-pipeline.mjs'], { cwd: ROOT, env: dupNumEnv, encoding: 'utf-8', timeout: 30000, stdio: ['pipe', 'pipe', 'pipe'] });
  ...
  try {
    ...
    const cleanOut = run(NODE, ['src/scripts/verify-pipeline.mjs'], { env: { ...process.env, CAREER_OPS_TRACKER: cleanTracker }, stdio: ['pipe', 'pipe', 'pipe'] });
...
// src/scripts/dedup-tracker.mjs used to ship an older fuzzy role matcher than
...
try {
  const { roleFuzzyMatch, roleTokens } = await import(pathToFileURL(join(ROOT, 'src/lib/role-matcher.mjs')).href);
  ...
  try {
    ...
    const dedupResult = run(NODE, ['src/scripts/dedup-tracker.mjs'], { env: { ...process.env, CAREER_OPS_TRACKER: tracker } });
    if (dedupResult === null) {
      fail('src/scripts/dedup-tracker.mjs crashed during shared role matcher safety test');
    } else {
...
// Any argv token src/scripts/dedup-tracker.mjs didn't recognize used to fall straight
...
try {
  ...
  try {
    ...
    const helpResult = run(NODE, ['src/scripts/dedup-tracker.mjs', '--help'], { env });
    ...
    const hResult = run(NODE, ['src/scripts/dedup-tracker.mjs', '-h'], { env });
    ...
    // write path.
    writeFileSync(tracker, seedTracker);
    const checkResult = run(NODE, ['src/scripts/dedup-tracker.mjs', '--check'], { env });
    ...
    // short-circuits, otherwise `src/scripts/dedup-tracker.mjs --help --check` would
    // exit 0 instead of erroring.
    writeFileSync(tracker, seedTracker);
    const mixedResult = run(NODE, ['src/scripts/dedup-tracker.mjs', '--help', '--check'], { env });
    ...
    // does not write).
    writeFileSync(tracker, seedTracker);
    const dryRunResult = run(NODE, ['src/scripts/dedup-tracker.mjs', '--dry-run'], { env });
    if (dryRunResult === null) {
      fail('src/scripts/dedup-tracker.mjs --dry-run crashed after the flag-validation fix');
    ...
    writeFileSync(tracker, seedTracker);
    const liveResult = run(NODE, ['src/scripts/dedup-tracker.mjs'], { env });
    if (liveResult === null) {
      fail('src/scripts/dedup-tracker.mjs (no flags) crashed after the flag-validation fix');
    } else {
...
try {
  ...
  try {
    ...
    const r = run(NODE, ['src/scripts/dedup-tracker.mjs'], { env: { ...process.env, CAREER_OPS_TRACKER: tracker } });
    if (r === null) {
      fail('src/scripts/dedup-tracker.mjs crashed during blind-via channel key test (#2393)');
    } else {
...
try {
  ...
  try {
    ...
    const r = run(NODE, ['src/scripts/dedup-tracker.mjs'], { env: { ...process.env, CAREER_OPS_TRACKER: tracker } });
    if (r === null) {
      fail('src/scripts/dedup-tracker.mjs crashed during non-Latin company key test (#2429)');
    } else {
      const out = readFileSync(tracker, 'utf-8');
...
try {
  ...
  try {
    ...
    const vpKeyOut = run(NODE, ['src/scripts/verify-pipeline.mjs'], { env: vpKeyEnv, stdio: ['pipe', 'pipe', 'pipe'] });
    if (vpKeyOut === null) {
...
try {
  ...
  try {
    ...
    const r = run(NODE, ['src/scripts/dedup-tracker.mjs'], { env: { ...process.env, CAREER_OPS_TRACKER: tracker } });
    if (r === null) {
      fail('src/scripts/dedup-tracker.mjs crashed during notes-preservation test');
    } else {
      const out = readFileSync(tracker, 'utf-8');
...
// opening three files. src/scripts/find.mjs resolves a report#, tracker#, or company/role
// fragment to the full pipeline identity in one read-only lookup.
console.log('\n🧪 Testing src/scripts/find.mjs pipeline identity lookup...');
try {
  const { parseTrackerRows, parsePdfIndex, findMatches } = await import(pathToFileURL(join(ROOT, 'src/scripts/find.mjs')).href);
  ...
  if (byTracker.length === 1 && byTracker[0].company === 'Acme Labs' &&
    ...
    pass('src/scripts/find.mjs resolves a tracker# to company, report#, canonical status, and PDF path');
  } else {
    fail(`src/scripts/find.mjs tracker# lookup wrong: ${JSON.stringify(byTracker)}`);
  }
  ...
  if (ambiguous.length === 2 && companies[0] === 'Acme Labs' && companies[1] === 'Globex') {
    pass('src/scripts/find.mjs surfaces report#/tracker# collisions as multiple matches (zero-pad normalized)');
  } else {
    fail(`src/scripts/find.mjs numeric collision lookup wrong: ${JSON.stringify(ambiguous)}`);
  }

  const byFragment = findMatches(rows, 'acme', pdfIndex);
  if (byFragment.length === 1 && byFragment[0].company === 'Acme Labs') {
    pass('src/scripts/find.mjs matches a case-insensitive company fragment');
  } else {
    fail(`src/scripts/find.mjs company fragment lookup wrong: ${JSON.stringify(byFragment)}`);
  }

  // Fuzzy multi-word lookup reuses src/lib/role-matcher.mjs (stopwords like "remote"
  ...
  if (byFuzzy.length === 1 && byFuzzy[0].company === 'Globex' && byFuzzy[0].pdfPath === null) {
    pass('src/scripts/find.mjs fuzzy-matches a role phrase via role-matcher and reports a missing PDF');
  } else {
    fail(`src/scripts/find.mjs fuzzy role lookup wrong: ${JSON.stringify(byFuzzy)}`);
  }

  if (findMatches(rows, 'no-such-company', pdfIndex).length === 0) {
    pass('src/scripts/find.mjs returns zero matches cleanly for an unknown query');
  } else {
    fail('src/scripts/find.mjs matched a query that exists nowhere in the tracker');
  }
} catch (e) {
  fail(`src/scripts/find.mjs unit test crashed: ${e.message}`);
}
...
try {
  const { roleFuzzyMatch } = await import(pathToFileURL(join(ROOT, 'src/lib/role-matcher.mjs')).href);
...
try {
  const { roleTokens, roleFuzzyMatch } = await import(pathToFileURL(join(ROOT, 'src/lib/role-matcher.mjs')).href);
...
try {
  ...
  try {
    ...
    let badCode = 0;
    try {
      badOut = execFileSync(NODE, ['src/scripts/verify-pipeline.mjs'], {
    ...
    let hdrCode = 0;
    try {
      hdrOut = execFileSync(NODE, ['src/scripts/verify-pipeline.mjs'], {
        cwd: ROOT, encoding: 'utf-8', timeout: 30000,
...
try {
  const { buildHtml } = await import(pathToFileURL(join(ROOT, 'src/scripts/generate-cover-letter.mjs')).href);
...
try {
  ...
  const usageOut = execFileSync(process.execPath, [join(ROOT, 'src/scripts/generate-cover-letter.mjs'), '--help'], { encoding: 'utf-8' });
...
// Run src/scripts/generate-latex.mjs and return its JSON report, capturing stdout even
...
function latexValidate(tex) {
  ...
  let out;
  try {
    out = execFileSync(NODE, ['src/scripts/generate-latex.mjs', texPath], { cwd: ROOT, encoding: 'utf-8', timeout: 30000 });
  } catch (e) {
...
try {
  const { parsePortals } = await import(pathToFileURL(join(ROOT, 'src/scripts/openrouter-runner.mjs')).href);
...
console.log('\n44b. openrouter-runner — prompt-cache breakpoint (#1709)');
try {
  const { buildCachedSystemMessage } = await import(pathToFileURL(join(ROOT, 'src/scripts/openrouter-runner.mjs')).href);
...
// src/scripts/openai-eval.mjs runs on import (arg parse + fetch), so it can't be imported to
...
console.log('\n44c. openai-eval — host-gated prompt-cache breakpoint (#1709)');
try {
  const src = readFileSync(join(ROOT, 'src/scripts/openai-eval.mjs'), 'utf-8');
  const checks = [
...
console.log('\n44d. gemini-eval — static prefix as systemInstruction (#1709)');
try {
  const src = readFileSync(join(ROOT, 'src/scripts/gemini-eval.mjs'), 'utf-8');
...
// src/scripts/openai-tailor.mjs runs on import (arg parse + fetch), so it can't be imported
...
console.log('\n44f. openai-tailor — host-gated prompt-cache breakpoint (#1709, #2432)');
try {
  const src = readFileSync(join(ROOT, 'src/scripts/openai-tailor.mjs'), 'utf-8');
  const checks = [
    // api.openai.com gets a plain-string system message (auto-caches; may reject the field)
...
try {
  const src = readFileSync(join(ROOT, 'src/scripts/ollama-eval.mjs'), 'utf-8');
...
try {
  ...
  const vreg = await import(pathToFileURL(join(ROOT, 'src/scripts/validate-plugin-registry.mjs')).href);
  const audit = await import(pathToFileURL(join(ROOT, 'src/scripts/plugin-audit.mjs')).href);
  const install = await import(pathToFileURL(join(ROOT, 'src/scripts/plugin-install.mjs')).href);
  ...
  if (["'plugins/'", "'src/scripts/plugins.mjs'", "'config/plugins.example.yml'"].every(s => upd.includes(s))) pass('plugins/, src/scripts/plugins.mjs, config/plugins.example.yml registered as SYSTEM paths');
...
// ── src/lib/match-star.mjs — fixture story-bank + top match assertion ───────────────

console.log('\n🧪 Testing src/lib/match-star.mjs keyword scorer...');

try {
  // Import the real production functions — tests exercise actual implementation
  const { parseStories, tokenize, score } = await import(pathToFileURL(join(ROOT, 'src/lib/match-star.mjs')).href);
...
`.trim();
  ...
  // src/lib/match-star.mjs file must exist (existsSync-guarded in the script itself)
  if (existsSync(join(ROOT, 'src/lib/match-star.mjs'))) {
    pass('src/lib/match-star.mjs: file present in repo root');
  } else {
    fail('src/lib/match-star.mjs: file missing from repo root');
  }
...
try {
  const src = readFile('src/scripts/prepare-application.mjs');

  // Must not make any network requests
  if (!/\bfetch\s*\(/.test(src) && !/https?\.request/.test(src) && !/createConnection/.test(src)) {
    pass('src/scripts/prepare-application.mjs makes no network requests');
  } else {
    fail('src/scripts/prepare-application.mjs calls a network API — must be prefill-only, no POST');
  }
  ...
  for (const fn of ['buildGreenhouseFields', 'buildAshbyFields', 'buildLeverFields']) {
    if (new RegExp(`function ${fn}`).test(src)) {
      pass(`src/scripts/prepare-application.mjs defines ${fn}`);
    } else {
      fail(`src/scripts/prepare-application.mjs missing concrete handler: ${fn}`);
    }
  }
  ...
  if (allowedHostsOk && levOk) {
    pass('src/scripts/prepare-application.mjs allowlists jobs.eu.lever.co in ALLOWED_HOSTS and detectAts() LEV set');
  } else {
    const missing = [!allowedHostsOk && 'ALLOWED_HOSTS', !levOk && 'LEV'].filter(Boolean).join(', ');
    fail(`src/scripts/prepare-application.mjs missing jobs.eu.lever.co from: ${missing}`);
  }

  // Must read config/profile.yml
  if (/config\/profile\.yml/.test(src)) {
    pass('src/scripts/prepare-application.mjs reads config/profile.yml');
  } else {
    fail('src/scripts/prepare-application.mjs does not read config/profile.yml');
  }
  ...
  if (/output[^'"`\n]*startsWith|startsWith.*output|relative\(outputDir/.test(src)) {
    pass('src/scripts/prepare-application.mjs restricts PDF path to output/');
  } else {
    fail('src/scripts/prepare-application.mjs missing output/ directory restriction for --pdf');
  }

  // Must enforce https-only
  if (/protocol.*https:|https:.*protocol/.test(src)) {
    pass('src/scripts/prepare-application.mjs enforces https-only URLs');
  } else {
    fail('src/scripts/prepare-application.mjs missing https enforcement');
  }

  // Must not reference old script name
  if (!/submit-resume/.test(src)) {
    pass('src/scripts/prepare-application.mjs does not reference old submit-resume name');
  } else {
    fail('src/scripts/prepare-application.mjs still references submit-resume');
  }
  ...
  if (/prepare.application.*prepare-application\.mjs/.test(pkg)) {
    ...
    fail('package.json missing prepare:application script pointing to src/scripts/prepare-application.mjs');
  }
...
console.log('\n59. CV template resolver (src/lib/cv-templates.mjs)');
{
  ...
  const resolved = run(NODE, ['src/lib/cv-templates.mjs', 'resolve', 'cv'], noProfile);
...
try {
  const src = readFileSync(join(ROOT, 'tests/run-all.mjs'), 'utf-8');
  ...
  if (!src.includes(emDash) && !src.includes(hyphen)) {
    pass('no provider sections re-added to tests/run-all.mjs');
  } else {
    fail('provider test section found in tests/run-all.mjs — add a tests/providers/{name}.test.mjs file instead (auto-discovered, no registration)');
  }
...
// src/scripts/salary-gap.mjs's own --self-test (invoked above via the CLI-check table)
...
try {
  ...
  if (planMode.includes('--stated-for') && planMode.includes('src/scripts/salary-gap.mjs')) {
    pass('interview/plan reads prior stated-comp observations via src/scripts/salary-gap.mjs --stated-for');
  } else {
  ...
  if (prepModeDoc.includes('--stated-for') && prepModeDoc.includes('src/scripts/salary-gap.mjs')) {
    pass('interview-prep reads prior stated-comp observations via src/scripts/salary-gap.mjs --stated-for');
  } else {
...
// existing round-type **Format:** field, cross-referencing src/scripts/invite-match.mjs's
...
try {
  ...
  if (processOverviewFlat.includes("extractPlatform") && processOverviewFlat.includes('src/scripts/invite-match.mjs')) {
...
// src/scripts/check-table-freshness.mjs's own --self-test (invoked above via the
...
try {
  const freshnessSrc = readFile('src/scripts/check-table-freshness.mjs');
  ...
  if (pkg.scripts && pkg.scripts.freshness === 'node src/scripts/check-table-freshness.mjs') {
  ...
  const agentsDoc = readFile('AGENTS.md');
  if (agentsDoc.includes('`src/scripts/check-table-freshness.mjs`')) {
    pass('AGENTS.md Main Files table lists src/scripts/check-table-freshness.mjs');
  } else {
    fail('AGENTS.md Main Files table missing src/scripts/check-table-freshness.mjs');
  }
  ...
  if (fsImports.length > 0 && nonWhitelisted.length === 0) {
    pass('src/scripts/check-table-freshness.mjs fs imports are read-only (readFileSync/readdirSync/existsSync only)');
  } else {
    fail(`src/scripts/check-table-freshness.mjs fs import boundary violated: ${nonWhitelisted.join(', ') || 'no fs import matched'}`);
  }
  if (!/from\s*['"](?:node:)?fs\/promises['"]/.test(freshnessSrc)) {
    pass('src/scripts/check-table-freshness.mjs does not import fs/promises');
  } else {
    fail('src/scripts/check-table-freshness.mjs imports fs/promises — write-capable API surface');
  }
  if (!/\brequire\s*\(/.test(freshnessSrc)) {
    pass('src/scripts/check-table-freshness.mjs has no require() escape hatch');
  } else {
    fail('src/scripts/check-table-freshness.mjs uses require() — bypasses the import whitelist');
  }
  if (!/import\s*\(\s*['"](?:node:)?fs/.test(freshnessSrc)) {
    pass('src/scripts/check-table-freshness.mjs has no dynamic fs import');
  } else {
    fail('src/scripts/check-table-freshness.mjs dynamically imports fs — bypasses the import whitelist');
  }
  ...
  if (foundWrite.length === 0) {
    pass('src/scripts/check-table-freshness.mjs contains no write-capable fs tokens');
  } else {
    fail(`src/scripts/check-table-freshness.mjs mentions write-capable fs APIs: ${foundWrite.join(', ')}`);
  }
} catch (e) {
...
// src/scripts/rejection-latency.mjs's own --self-test (invoked above via the CLI-check
...
try {
  const followupModeDoc = readFile('modes/followup.md');
  const rejectionLatencySrc = readFile('src/scripts/rejection-latency.mjs');

  if (followupModeDoc.includes('src/scripts/rejection-latency.mjs')) {
    pass('followup mode runs src/scripts/rejection-latency.mjs and surfaces flags as reminders');
  } else {
    fail('followup mode missing the src/scripts/rejection-latency.mjs check step');
  }
  ...
  if (!/STATUTORY_THRESHOLDS|resolveJurisdiction|--jurisdiction/.test(rejectionLatencySrc)) {
    pass('src/scripts/rejection-latency.mjs carries no statutory tier (courtesy-only, CodeRabbit PR #2014 review)');
  } else {
    fail('src/scripts/rejection-latency.mjs still references the removed statutory tier / jurisdiction resolution');
  }

  if (rejectionLatencySrc.includes('not legal advice')) {
    pass('src/scripts/rejection-latency.mjs carries the not-legal-advice disclaimer');
  } else {
    fail('src/scripts/rejection-latency.mjs missing the not-legal-advice disclaimer');
  }
  ...
  if (fsImports.length > 0 && fsImportViolations.length === 0) {
    pass('src/scripts/rejection-latency.mjs fs imports are restricted to read-only APIs (readFileSync/existsSync)');
  } else {
    fail(`src/scripts/rejection-latency.mjs fs import surface is not read-only: ${fsImportViolations.join(', ') || 'no fs import found'}`);
  }

  if (!/\brequire\s*\(/.test(rejectionLatencySrc) && !/import\s*\(\s*['"](?:node:)?fs/.test(rejectionLatencySrc)) {
    pass('src/scripts/rejection-latency.mjs has no require() or dynamic fs import escape hatch');
  } else {
    fail('src/scripts/rejection-latency.mjs uses require() or a dynamic fs import — read-only guarantee not verifiable');
  }
  ...
  if (!/\b(?:writeFileSync|writeFile|appendFileSync|appendFile|createWriteStream|openSync|writeSync|unlinkSync|rmSync|rmdirSync|mkdirSync|renameSync|truncateSync|copyFileSync)\b/.test(rejectionLatencySrc)) {
    pass('src/scripts/rejection-latency.mjs references no filesystem mutation APIs (suggestion-only by construction)');
  } else {
    fail('src/scripts/rejection-latency.mjs contains file-write APIs; it must never write user data');
  }
} catch (e) {
