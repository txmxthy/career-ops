...
/**
 * tests/paste-reply.test.mjs — regression tests for src/scripts/paste-reply.mjs (#1802).
 *
 * Locks in the manual/no-Gmail input path into src/scripts/reply-watch.mjs's classification
 * pipeline:
 *   1. --file mode normalizes subject/from/body into the exact candidate shape
 *      src/scripts/reply-watch.mjs expects, with signal left null (classification stays
 *      src/scripts/reply-watch.mjs's job).
...
console.log('1. --file mode normalizes into the exact candidate shape');
{
  ...
  check('signal is null (classification stays src/scripts/reply-watch.mjs job)', cand.signal === null, JSON.stringify(cand.signal));
