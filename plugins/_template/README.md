...
node src/scripts/plugins.mjs add {{NAME}}

# Before listing (install directly from your repo at a pinned commit):
node src/scripts/plugins.mjs add <your-github-user>/career-ops-plugin-{{NAME}} --sha <40-hex-commit>
```

Then enable + consent:

```bash
node src/scripts/plugins.mjs enable {{NAME}}            # shows the capability card
node src/scripts/plugins.mjs enable {{NAME}} --confirm  # grants it
```
