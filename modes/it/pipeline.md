...
1. **Playwright (preferito):** `browser_navigate` + `browser_snapshot`. Funziona con tutte le Single Page Application (SPA).
   - **Opzionale — estrattore CLI (`scan.extractor: cli` in `config/profile.yml`):** esegui invece `node src/scripts/browser-extract.mjs <url>` (`--mode jd`) — `{ "url", "title", "text" }` compatto, meno token (dipende dal portale). **Ripiega in silenzio** su `browser_navigate` + `browser_snapshot` in caso di errore o assenza.
