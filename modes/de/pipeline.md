...
1. **Playwright (bevorzugt):** `browser_navigate` + `browser_snapshot`. Funktioniert mit allen SPAs.
   - **Opt-in — CLI-Extraktor (`scan.extractor: cli` in `config/profile.yml`):** stattdessen `node src/scripts/browser-extract.mjs <url>` (`--mode jd`) ausführen — kompaktes `{ "url", "title", "text" }`, weniger Tokens (je nach Jobportal). Bei Fehler oder wenn es fehlt **still** auf `browser_navigate` + `browser_snapshot` zurückfallen.
