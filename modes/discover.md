...
```bash
node src/scripts/discover-ats.mjs --in companies.yml
```
...
```bash
node src/scripts/discover-ats.mjs --in companies.yml --write
```

Other forms:

```bash
node src/scripts/discover-ats.mjs Stripe Ramp Mollie          # names as positional args
node src/scripts/discover-ats.mjs --in companies.yml --summary # human-readable table
node src/scripts/discover-ats.mjs --in companies.yml --vendors gh,ashby  # restrict probes
