...
1. **Playwright (foretrukket):** `browser_navigate` + `browser_snapshot`. Fungerer med alle SPAs.
   - **Tilvalg — CLI-udtrækker (`scan.extractor: cli` i `config/profile.yml`):** kør `node src/scripts/browser-extract.mjs <url>` (`--mode jd`) i stedet — kompakt `{ "url", "title", "text" }`, færre tokens (afhænger af jobportalen). **Fald stille tilbage** til `browser_navigate` + `browser_snapshot` ved fejl eller hvis den mangler.
