...
1. **أداة Playwright (الخيار المفضل):** استخدام أداة التنقل وأخذ لقطة للصفحة (`browser_navigate` + `browser_snapshot`). تعمل بكفاءة عالية مع كافة تطبيقات الصفحة الواحدة (SPAs).
   - **اختياري — أداة استخراج CLI (`scan.extractor: cli` في `config/profile.yml`):** شغّل `node src/scripts/browser-extract.mjs <url>` (`--mode jd`) بدلًا من ذلك — `{ "url", "title", "text" }` مضغوط، عدد توكنات أقل (حسب الموقع). عند الفشل أو الغياب **ارجع بصمت** إلى `browser_navigate` + `browser_snapshot`.
